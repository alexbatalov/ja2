...
void PrintTime(char *output, FILETIME TimeToPrint) {
  ...
  if (FileTimeToLocalFileTime(&TimeToPrint, &TimeToPrint) && FileTimeToDosDateTime(&TimeToPrint, &Date, &Time)) {
    ...
    wsprintf(output, "%02d/%02d/%d %02d:%02d:%02d", (Date / 32) & 15, Date & 31, (Date / 512) + 1980, (Time / 2048), (Time / 32) & 63, (Time & 31) * 2);
  } else
    output[0] = 0;
}
...
static void ShowModuleInfo(HWFILE hFile, HINSTANCE ModuleHandle) {
  ...
  __try {
    if (GetModuleFileName(ModuleHandle, ModName, sizeof(ModName)) > 0) {
      ...
      // and size.
      HANDLE ModuleFile = CreateFile(ModName, GENERIC_READ, FILE_SHARE_READ, 0, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0);
      char TimeBuffer[100] = "";
      DWORD FileSize = 0;
