...
// ATE: Insert for demo , not using the heli sequence....
  ...
  if (pHireMerc.uiTimeTillMercArrives != 0) {
    AddStrategicEvent(Enum132.EVENT_DELAYED_HIRING_OF_MERC, pHireMerc.uiTimeTillMercArrives, pSoldier.ubID);
...
export function MercArrivesCallback(ubSoldierID: UINT8): void {
  ...
  if (gWorldSectorX == pSoldier.sSectorX && gWorldSectorY == pSoldier.sSectorY && pSoldier.bSectorZ == gbWorldSectorZ) {
    ...
    // Setup chopper....
    if (pSoldier.ubStrategicInsertionCode != Enum175.INSERTION_CODE_CHOPPER && pSoldier.sSectorX == 9 && pSoldier.sSectorY == 1) {
  ...
  if (pSoldier.ubStrategicInsertionCode != Enum175.INSERTION_CODE_CHOPPER) {
    ScreenMsg(FONT_MCOLOR_WHITE, MSG_INTERFACE, TacticalStr[Enum335.MERC_HAS_ARRIVED_STR], pSoldier.name);
  ...
  if (pSoldier.bTeam == gbPlayerNum) {
    ...
    if (SoldierHasWorseEquipmentThanUsedTo(pSoldier)) {
      ...
      if (GetWorldMinutesInDay() < uiTimeOfPost) {
        AddSameDayStrategicEvent(Enum132.EVENT_MERC_COMPLAIN_EQUIPMENT, uiTimeOfPost, pSoldier.ubProfile);
      }
    }
  }
