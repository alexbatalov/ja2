...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("MANLOOKSFOROTHERTEAMS ID %d(%S) team %d side %d", pSoldier->ubID, pSoldier->name, pSoldier->bTeam, pSoldier->bSide));
#endif
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("ERROR: ManLooksForMan - WE are inactive/dead etc ID %d(%S)to ID %d", pSoldier->ubID, pSoldier->name, pOpponent->ubID));
#endif
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("ERROR: ManLooksForMan - TARGET is inactive etc ID %d(%S)to ID %d", pSoldier->ubID, pSoldier->name, pOpponent->ubID));
#endif
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("ERROR: ManLooksForMan - SAME TEAM ID %d(%S)to ID %d", pSoldier->ubID, pSoldier->name, pOpponent->ubID));
#endif
...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("OTHERTEAMSLOOKFORMAN ID %d(%S) team %d side %d", pOpponent->ubID, pOpponent->name, pOpponent->bTeam, pOpponent->bSide));
#endif
...
#ifdef TESTOPPLIST
  DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("RADIO SIGHTINGS: for %d about %d", pSoldier->ubID, ubAbout));
#endif
...
#ifdef TESTOPPLIST
      DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("RS: inactive/notInSector/life %d", pOpponent->ubID));
#endif
...
#ifdef TESTOPPLIST
    DebugMsg(TOPIC_JA2OPPLIST, DBG_LEVEL_3, String("...............UPDATE PUBLIC: soldier %d SEEING soldier %d", pSoldier->ubID, pOpponent->ubID));
#endif
...
void DebugSoldierPage2() {
  ...
  if (FindSoldierFromMouse(&usSoldierIndex, &uiMercFlags)) {
    ...
    if (pSoldier->bTeam == OUR_TEAM || pSoldier->bTeam == MILITIA_TEAM) // look at 8 to 15 opplist entries
    {
      ...
      SetFontShade(LARGEFONT1, FONT_SHADE_NEUTRAL);
      gprintf(150, LINE_HEIGHT * ubLine, L"%d %d %d %d %d %d %d %d", pSoldier->bOppList[20], pSoldier->bOppList[21], pSoldier->bOppList[22], pSoldier->bOppList[23], pSoldier->bOppList[24], pSoldier->bOppList[25], pSoldier->bOppList[26], pSoldier->bOppList[27]);
      ubLine++;
    } else // team 1 - enemies so look at first 8 (0-7) opplist entries
    {
      ...
      SetFontShade(LARGEFONT1, FONT_SHADE_NEUTRAL);
      gprintf(150, LINE_HEIGHT * ubLine, L"%d %d %d %d %d %d %d %d", pSoldier->bOppList[0], pSoldier->bOppList[1], pSoldier->bOppList[2], pSoldier->bOppList[3], pSoldier->bOppList[4], pSoldier->bOppList[5], pSoldier->bOppList[6], pSoldier->bOppList[7]);
      ubLine++;
    }
    ...
    SetFontShade(LARGEFONT1, FONT_SHADE_NEUTRAL);
    gprintf(150, LINE_HEIGHT * ubLine, L"%d %d %d", pSoldier->usPathingData[0], pSoldier->usPathingData[1], pSoldier->usPathingData[2]);
    ubLine++;

    SetFontShade(LARGEFONT1, FONT_SHADE_GREEN);
    gprintf(0, LINE_HEIGHT * ubLine, L"Next 3 Steps:");
    SetFontShade(LARGEFONT1, FONT_SHADE_NEUTRAL);
    gprintf(150, LINE_HEIGHT * ubLine, L"%d %d %d", pSoldier->usPathingData[pSoldier->usPathIndex], pSoldier->usPathingData[pSoldier->usPathIndex + 1], pSoldier->usPathingData[pSoldier->usPathIndex + 2]);
    ubLine++;
...
void DebugSoldierPage3() {
  ...
  if (FindSoldierFromMouse(&usSoldierIndex, &uiMercFlags)) {
    ...
    if (gubWatchedLocPoints[pSoldier->ubID][0] > 0) {
      gprintf(350, LINE_HEIGHT * ubLine, L"Watch %d/%d for %d pts", gsWatchedLoc[pSoldier->ubID][0], gbWatchedLocLevel[pSoldier->ubID][0], gubWatchedLocPoints[pSoldier->ubID][0]);
    }
    ...
    if (gubWatchedLocPoints[pSoldier->ubID][1] > 0) {
      gprintf(350, LINE_HEIGHT * ubLine, L"Watch %d/%d for %d pts", gsWatchedLoc[pSoldier->ubID][1], gbWatchedLocLevel[pSoldier->ubID][1], gubWatchedLocPoints[pSoldier->ubID][1]);
    }
    ...
    if (gubWatchedLocPoints[pSoldier->ubID][2] > 0) {
      gprintf(350, LINE_HEIGHT * ubLine, L"Watch %d/%d for %d pts", gsWatchedLoc[pSoldier->ubID][2], gbWatchedLocLevel[pSoldier->ubID][2], gubWatchedLocPoints[pSoldier->ubID][2]);
    }
...
void WriteQuantityAndAttachments(OBJECTTYPE *pObject, INT32 yp) {
  ...
  if (pObject->ubNumberOfObjects > 1 && fAttachments) {
    // everything
    gprintf(320, yp, L"%d%%  Qty:  %d  %s", pObject->bStatus[0], pObject->ubNumberOfObjects, szAttach);
...
void DebugSoldierPage4() {
  ...
  if (FindSoldierFromMouse(&usSoldierIndex, &uiMercFlags)) {
    ...
    if (pSoldier->bTeam != OUR_TEAM) {
      ...
      SetFontShade(LARGEFONT1, FONT_SHADE_NEUTRAL);
      if (pNode) {
        gprintf(0, LINE_HEIGHT * ubLine, L"%s, %s, REL EQUIP: %d, REL ATTR: %d", szOrders, szAttitude, pNode->pBasicPlacement->bRelativeEquipmentLevel, pNode->pBasicPlacement->bRelativeAttributeLevel);
      } else {
...
#ifdef BETAVERSION
  sprintf(tempstr, "OurNoise: ubNoiseType = %s, ubNoiseMaker = %d, ubNoiseMode = %d, sGridNo = %d, ubVolume = %d", NoiseTypeStr[ubNoiseType], ubNoiseMaker, ubNoiseMode, sGridNo, ubVolume);
#ifdef RECORDNET
...
#ifdef BETAVERSION
  sprintf(tempstr, "TheirNoise: ubNoiseType = %s, ubNoiseMaker = %d, ubNoiseMode = %d, sGridNo = %d, ubVolume = %d", NoiseTypeStr[ubNoiseType], ubNoiseMaker, ubNoiseMode, sGridNo, ubVolume);
#ifdef RECORDNET
  fprintf(NetDebugFile, "\t%s\n", tempstr);
#endif
...
#ifdef RECORDOPPLIST
    fprintf(OpplistFile, "HN: %s by %2d(g%4d,x%3d,y%3d) at %2d(g%4d,x%3d,y%3d), hTT=%d\n", (bSourceSeen) ? "SCS" : "FLR", pSoldier->guynum, pSoldier->sGridNo, pSoldier->sX, pSoldier->sY, ubNoiseMaker, sGridNo, sNoiseX, sNoiseY, bHadToTurn);
#endif
  }
...
#ifdef BETAVERSION
    sprintf(tempstr, "VerifyAndDecayOpplist: WARNING - %d(%s) still has %d NEW OPPONENTS - lastCaller %s/%s", pSoldier->guynum, ExtMen[pSoldier->guynum].name, pSoldier->newOppCnt, LastCallerText[ExtMen[pSoldier->guynum].lastCaller], LastCaller2Text[ExtMen[pSoldier->guynum].lastCaller2]);
