...
export let Item: INVTYPE[] /* [MAXITEMS] */ = [
  ...
  //---------				-----		-------     -------		----	--	  --  ----  -----	----	-----------							---		------		-----
  createInvTypeFrom(IC_PUNCH, 0, PUNCHCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, ITEM_NOT_BUYABLE),
  //---WEAPONS---
  ...
  createInvTypeFrom(IC_GUN, 1, TARGETCURS, CONDBUL, 0, 1, 6, 1, 350, 2, /* Glock 17        */ +2, +2, IF_STANDARD_GUN),
  createInvTypeFrom(IC_GUN, 2, TARGETCURS, CONDBUL, 0, 2, 6, 1, 480, 2, /* Glock 18        */ +1, +1, IF_STANDARD_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 3, TARGETCURS, CONDBUL, 0, 3, 11, 1, 450, 2, /* Beretta 92F     */ -1, -1, IF_STANDARD_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 4, TARGETCURS, CONDBUL, 0, 4, 11, 1, 470, 2, /* Beretta 93R     */ -2, -2, IF_STANDARD_GUN),
  createInvTypeFrom(IC_GUN, 5, TARGETCURS, CONDBUL, 0, 5, 11, 1, 250, 1, /* .38 S&W Special */ +4, +4, IF_STANDARD_GUN),
  createInvTypeFrom(IC_GUN, 6, TARGETCURS, CONDBUL, 0, 6, 10, 1, 300, 1, /* .357 Barracuda  */ +3, +3, IF_STANDARD_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 7, TARGETCURS, CONDBUL, 0, 7, 17, 1, 300, 1, /* .357 DesertEagle*/ -1, -1, IF_STANDARD_GUN),
  createInvTypeFrom(IC_GUN, 8, TARGETCURS, CONDBUL, 0, 8, 11, 1, 400, 2, /* .45 M1911			 */ 0, 0, IF_STANDARD_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 9, TARGETCURS, CONDBUL, 0, 9, 21, 0, 980, 3, /* H&K MP5K      	 */ -1, 0, IF_STANDARD_GUN),
  createInvTypeFrom(IC_GUN, 10, TARGETCURS, CONDBUL, 0, 10, 28, 0, 1170, 4, /* .45 MAC-10	     */ -2, -1, IF_STANDARD_GUN),

  createInvTypeFrom(IC_GUN, 11, TARGETCURS, CONDBUL, 0, 11, 48, 0, 700, 3, /* Thompson M1A1   */ +3, -3, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 12, TARGETCURS, CONDBUL, 0, 12, 26, 0, 1330, 5, /* Colt Commando   */ 0, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 13, TARGETCURS, CONDBUL, 0, 13, 31, 0, 770, 3, /* H&K MP53		 		 */ -1, -1, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 14, TARGETCURS, CONDBUL, 0, 14, 39, 0, 1180, 4, /* AKSU-74         */ -2, -1, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 15, TARGETCURS, CONDBUL, 0, 15, 28, 0, 2750, 9, /* 5.7mm FN P90    */ -2, -4, IF_STANDARD_GUN | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GUN, 16, TARGETCURS, CONDBUL, 0, 16, 19, 0, 620, 3, /* Type-85         */ -4, +2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 17, TARGETCURS, CONDBUL, 0, 17, 39, 0, 1350, 5, /* SKS             */ -4, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 18, TARGETCURS, CONDBUL, 0, 18, 43, 0, 1930, 6, /* Dragunov        */ +2, +2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 19, TARGETCURS, CONDBUL, 0, 19, 66, 0, 1950, 6, /* M24             */ +4, +4, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 20, TARGETCURS, CONDBUL, 0, 20, 36, 0, 2380, 8, /* Steyr AUG       */ +1, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),

  createInvTypeFrom(IC_GUN, 21, TARGETCURS, CONDBUL, 0, 21, 41, 0, 1620, 6, /* H&K G41         */ +1, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 22, TARGETCURS, CONDBUL, 0, 22, 29, 0, 1100, 4, /* Ruger Mini-14   */ 0, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 23, TARGETCURS, CONDBUL, 0, 23, 36, 0, 2680, 8, /* C-7             */ -1, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 24, TARGETCURS, CONDBUL, 0, 24, 36, 0, 1970, 7, /* FA-MAS          */ -2, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 25, TARGETCURS, CONDBUL, 0, 25, 36, 0, 1830, 6, /* AK-74           */ -1, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 26, TARGETCURS, CONDBUL, 0, 26, 43, 0, 1450, 5, /* AKM             */ +2, +2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 27, TARGETCURS, CONDBUL, 0, 27, 29, 0, 2120, 7, /* M-14            */ +1, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 28, TARGETCURS, CONDBUL, 0, 28, 43, 0, 2680, 8, /* FN-FAL          */ 0, -1, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 29, TARGETCURS, CONDBUL, 0, 29, 44, 0, 1570, 5, /* H&K G3A3        */ +1, -1, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 30, TARGETCURS, CONDBUL, 0, 30, 38, 0, 2530, 8, /* H&K G11         */ +3, -4, IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE),

  createInvTypeFrom(IC_GUN, 31, TARGETCURS, CONDBUL, 0, 31, 36, 0, 670, 3, /* Remington M870  */ +3, +3, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 32, TARGETCURS, CONDBUL, 0, 32, 38, 0, 980, 4, /* SPAS-15         */ -2, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 33, TARGETCURS, CONDBUL, 0, 33, 41, 0, 2900, 9, /* CAWS            */ -3, -3, IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GUN, 34, TARGETCURS, CONDBUL, 0, 34, 68, 0, 3100, 10, /* FN Minimi       */ -1, -2, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_GUN, 35, TARGETCURS, CONDBUL, 0, 35, 48, 0, 3180, 10, /* RPK-74          */ -1, -2, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_GUN, 36, TARGETCURS, CONDBUL, 0, 36, 93, 0, 3420, 10, /* H&K 21E         */ +2, +1, IF_TWOHANDED_GUN | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_BLADE, 37, KNIFECURS, 0, 1, 79, 9, 2, 100, 2, /* combat knife    */ +2, +2, IF_STANDARD_BLADE),
  createInvTypeFrom(IC_THROWING_KNIFE, 38, TARGETCURS, 0, 1, 53, 1, 4, 50, 3, /* throwing knife  */ -1, -1, IF_STANDARD_BLADE),
  createInvTypeFrom(IC_THROWN, 39, TOSSCURS, 0, 1, 57, 5, 2, 0, 0, /* rock            */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_LAUNCHER, 40, TRAJECTORYCURS, 0, 0, 37, 26, 0, 900, 7, /* grenade launcher*/ 0, -1, IF_TWOHANDED_GUN),

  createInvTypeFrom(IC_LAUNCHER, 41, TRAJECTORYCURS, 0, 0, 0, 77, 0, 1800, 10, /* mortar */ 0, -2, IF_TWOHANDED_GUN),
  createInvTypeFrom(IC_THROWN, 42, TOSSCURS, 0, 1, 60, 4, 3, 0, 0, /* another rock    */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_BLADE, 43, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* yng male claws */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_BLADE, 44, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* yng fem claws  */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_BLADE, 45, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* old male claws */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_BLADE, 46, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* old fem claws  */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_TENTACLES, 47, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* queen tentacles*/ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_GUN, 48, TARGETCURS, 0, 0, 0, 0, 1, 0, 0, /* queen spit */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_PUNCH, 49, PUNCHCURS, 0, 1, 102, 1, 4, 20, 2, /* brass knuckles */ 0, 0, IF_STANDARD_BLADE),
  createInvTypeFrom(IC_LAUNCHER, 50, INVALIDCURS, 0, 0, 39, 13, 0, 500, 8, /* underslung g.l.*/ 0, 0, IF_STANDARD_GUN),

  createInvTypeFrom(IC_GUN, 51, TARGETCURS, 0, 0, 38, 21, 0, 500, 9, /* rocket Launcher*/ 0, -3, IF_TWOHANDED_GUN), // now repairable
  createInvTypeFrom(IC_BLADE, 52, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* bloodcat claws*/ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_BLADE, 53, KNIFECURS, 0, 0, 0, 0, 1, 0, 0, /* bloodcat bite */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_BLADE, 54, KNIFECURS, 0, 0, 41, 13, 0, 200, 3, /* machete */ 0, +3, IF_STANDARD_BLADE),
  createInvTypeFrom(IC_GUN, 55, TARGETCURS, 0, 0, 45, 40, 0, 5000, 0, /* rocket rifle */ 0, -5, IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_GUN, 56, TARGETCURS, 0, 0, 40, 12, 0, 1000, 0, /* Automag III */ 0, -2, IF_STANDARD_GUN | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GUN, 57, TARGETCURS, 0, 0, 0, 0, 0, 0, 0, /* infant spit */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_GUN, 58, TARGETCURS, 0, 0, 0, 0, 0, 0, 0, /* yng male spit */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_GUN, 59, TARGETCURS, 0, 0, 0, 0, 0, 0, 0, /* old male spit */ 0, 0, ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_GUN, 60, TARGETCURS, 0, 0, 37, 26, 0, 0, 0, /* tank cannon  */ 0, 0, ITEM_NOT_BUYABLE),

  createInvTypeFrom(IC_GUN, 61, TARGETCURS, 0, 0, 46, 12, 1, 500, 5, /* dart gun */ 0, +3, IF_STANDARD_GUN),
  createInvTypeFrom(IC_THROWING_KNIFE, 62, TARGETCURS, 0, 1, 95, 1, 4, 50, 0, /*bloody throw.knife*/ 0, +4, IF_STANDARD_BLADE | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GUN, 63, TARGETCURS, 0, 0, 48, 18, 0, 0, 0, /* flamethrower */ 0, 0, IF_STANDARD_GUN | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_PUNCH, 64, PUNCHCURS, 0, 1, 85, 30, 0, 40, 1, /* Crowbar       */ 0, -4, ITEM_METAL | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_GUN, 65, TARGETCURS, 0, 0, 45, 40, 0, 10000, 0, /* rocket rifle */ 0, -5, IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  //---AMMO---
  createInvTypeFrom(IC_AMMO, 0, INVALIDCURS, 0, 1, 32, 2, 8, 15, 2, /* CLIP9_15 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 1, INVALIDCURS, 0, 1, 35, 3, 4, 30, 4, /* CLIP9_30 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 2, INVALIDCURS, 0, 1, 33, 2, 8, 45, 4, /* CLIP9_15_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 3, INVALIDCURS, 0, 1, 36, 3, 4, 90, 6, /* CLIP9_30_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 4, INVALIDCURS, 0, 1, 34, 2, 8, 30, 3, /* CLIP9_15_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 5, INVALIDCURS, 0, 1, 37, 3, 4, 60, 5, /* CLIP9_30_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 6, INVALIDCURS, 0, 1, 24, 1, 8, 5, 1, /* CLIP38_6 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 7, INVALIDCURS, 0, 1, 25, 1, 8, 15, 3, /* CLIP38_6_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 8, INVALIDCURS, 0, 1, 26, 1, 8, 10, 2, /* CLIP38_6_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 9, INVALIDCURS, 0, 1, 14, 2, 8, 10, 2, /* CLIP45_7 */ 0, 0, IF_STANDARD_CLIP),

  createInvTypeFrom(IC_AMMO, 10, INVALIDCURS, 0, 1, 4, 10, 4, 45, 3, /* CLIP45_30 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 11, INVALIDCURS, 0, 1, 15, 2, 8, 45, 4, /* CLIP45_7_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 12, INVALIDCURS, 0, 1, 5, 10, 4, 135, 5, /* CLIP45_30_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 13, INVALIDCURS, 0, 1, 16, 2, 8, 30, 3, /* CLIP45_7_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 14, INVALIDCURS, 0, 1, 6, 10, 4, 90, 4, /* CLIP45_30_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 15, INVALIDCURS, 0, 1, 11, 1, 8, 10, 1, /* CLIP357_6 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 16, INVALIDCURS, 0, 1, 17, 3, 8, 15, 1, /* CLIP357_9 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 17, INVALIDCURS, 0, 1, 12, 1, 8, 30, 3, /* CLIP357_6_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 18, INVALIDCURS, 0, 1, 18, 3, 8, 45, 3, /* CLIP357_9_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 19, INVALIDCURS, 0, 1, 13, 1, 8, 20, 2, /* CLIP357_6_HP */ 0, 0, IF_STANDARD_CLIP),

  createInvTypeFrom(IC_AMMO, 20, INVALIDCURS, 0, 1, 19, 3, 8, 30, 2, /* CLIP357_9_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 21, INVALIDCURS, 0, 1, 9, 6, 4, 150, 5, /* CLIP545_30_AP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 22, INVALIDCURS, 0, 1, 10, 6, 4, 100, 4, /* CLIP545_30_HP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 23, INVALIDCURS, 0, 1, 7, 5, 4, 150, 4, /* CLIP556_30_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 24, INVALIDCURS, 0, 1, 8, 5, 4, 100, 3, /* CLIP556_30_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 25, INVALIDCURS, 0, 1, 22, 3, 6, 60, 6, /* CLIP762W_10_AP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 26, INVALIDCURS, 0, 1, 29, 8, 4, 180, 4, /* CLIP762W_30_AP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 27, INVALIDCURS, 0, 1, 23, 3, 6, 40, 5, /* CLIP762W_10_HP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 28, INVALIDCURS, 0, 1, 30, 8, 4, 120, 3, /* CLIP762W_30_HP */ 0, 0, IF_STANDARD_CLIP | ITEM_BIGGUNLIST),
  createInvTypeFrom(IC_AMMO, 29, INVALIDCURS, 0, 1, 3, 1, 6, 30, 7, /* CLIP762N_5_AP */ 0, 0, IF_STANDARD_CLIP),

  createInvTypeFrom(IC_AMMO, 30, INVALIDCURS, 0, 1, 27, 8, 4, 120, 6, /* CLIP762N_20_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 31, INVALIDCURS, 0, 1, 2, 1, 6, 20, 6, /* CLIP762N_5_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 32, INVALIDCURS, 0, 1, 28, 8, 4, 80, 5, /* CLIP762N_20_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 33, INVALIDCURS, 0, 1, 31, 5, 4, 700, 8, /* CLIP47_50_SAP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 34, INVALIDCURS, 0, 1, 20, 9, 4, 750, 9, /* CLIP57_50_SAP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 35, INVALIDCURS, 0, 1, 21, 9, 4, 500, 9, /* CLIP57_50_HP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 37, INVALIDCURS, 0, 2, 22, 5, 6, 20, 3, /* CLIP12G_7 */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 36, INVALIDCURS, 0, 2, 4, 5, 6, 20, 3, /* CLIP12G_7_BUCKSHOT */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 39, INVALIDCURS, 0, 1, 0, 10, 6, 300, 9, /* CLIPCAWS_10_SAP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 38, INVALIDCURS, 0, 1, 1, 10, 6, 300, 9, /* CLIPCAWS_10_FLECH */ 0, 0, IF_STANDARD_CLIP),

  createInvTypeFrom(IC_AMMO, 40, INVALIDCURS, 0, 1, 110, 10, 4, 500, 9, /* CLIPROCKET_AP */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 41, INVALIDCURS, 0, 1, 115, 10, 4, 500, 9, /* CLIPROCKET_HE */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 42, INVALIDCURS, 0, 1, 114, 10, 4, 500, 9, /* CLIPROCKET_HEAT */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 43, INVALIDCURS, 0, 1, 119, 1, 8, 10, 4, /* sleep dart */ 0, 0, IF_STANDARD_CLIP),
  createInvTypeFrom(IC_AMMO, 44, INVALIDCURS, 0, 0, 49, 8, 4, 0, 0, /* flameThrwr clip */ 0, 0, IF_STANDARD_CLIP | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  ...
  createInvTypeFrom(IC_GRENADE, 0, TOSSCURS, 0, 1, 38, 6, 4, 100, 6, /* stun grenade				*/ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 1, TOSSCURS, 0, 1, 48, 6, 4, 120, 5, /* tear gas grenade   */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 2, TOSSCURS, 0, 1, 41, 6, 4, 500, 8, /* mustard gas grenade*/ 0, -3, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 3, TOSSCURS, 0, 1, 50, 3, 6, 150, 6, /* mini hand grenade  */ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 4, TOSSCURS, 0, 1, 49, 6, 4, 200, 7, /* reg hand grenade   */ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 5, INVALIDCURS, 0, 2, 3, 11, 2, 400, 7, /* RDX                */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 6, INVALIDCURS, 0, 2, 0, 11, 1, 500, 6, /* TNT (="explosives")*/ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 7, INVALIDCURS, 0, 2, 23, 11, 1, 1000, 8, /* HMX (=RDX+TNT)     */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 8, INVALIDCURS, 0, 1, 45, 11, 1, 750, 7, /* C1  (=RDX+min oil) */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 9, INVALIDCURS, 0, 1, 40, 41, 2, 400, 9, /* mortar shell       */ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),

  createInvTypeFrom(IC_BOMB, 10, BOMBCURS, 0, 1, 46, 8, 1, 300, 5, /* mine               */ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 11, INVALIDCURS, 0, 1, 44, 11, 1, 1500, 9, /* C4  ("plastique")  */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 12, BOMBCURS, 0, 1, 42, 4, 2, 0, 0, /* trip flare				  */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 13, BOMBCURS, 0, 1, 43, 4, 2, 0, 0, /* trip klaxon        */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 14, INVALIDCURS, 0, 1, 107, 2, 4, 250, 6, /* shaped charge			*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 15, TOSSCURS, 0, 2, 24, 1, 6, 50, 3, /* break light (flare)*/ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 16, INVALIDCURS, 0, 1, 97, 10, 4, 400, 8, /* 40mm HE grenade		*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 17, INVALIDCURS, 0, 1, 111, 10, 4, 250, 6, /* 40mm tear gas grnd */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 18, INVALIDCURS, 0, 1, 113, 10, 4, 200, 5, /* 40mm stun grenade  */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_GRENADE, 19, INVALIDCURS, 0, 1, 112, 10, 4, 100, 7, /* 40mm smoke grenade */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),

  createInvTypeFrom(IC_GRENADE, 20, TOSSCURS, 0, 1, 98, 6, 4, 50, 4, /* smoke hand grenade */ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_BOMB, 21, INVALIDCURS, 0, 1, 40, 41, 8, 450, 0, /* tank shell       */ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR),
  createInvTypeFrom(IC_BOMB, 22, INVALIDCURS, 0, 1, 40, 41, 2, 450, 0, /* fake struct ignite*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GRENADE, 23, TOSSCURS, 0, 2, 37, 6, 4, 50, 0, /* creature cocktail*/ 0, 0, ITEM_DAMAGEABLE | ITEM_METAL),
  createInvTypeFrom(IC_BOMB, 24, INVALIDCURS, 0, 1, 40, 41, 2, 450, 0, /* fake struct xplod*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_BOMB, 25, INVALIDCURS, 0, 1, 40, 41, 2, 450, 0, /* fake vehicle xplod*/ 0, -4, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GRENADE, 26, TOSSCURS, 0, 1, 48, 6, 4, 0, 0, /* BIG tear gas grenade*/ 0, -2, ITEM_DAMAGEABLE | ITEM_METAL | ITEM_REPAIRABLE | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_GRENADE, 27, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* small creature gas */ 0, 0, 0),
  createInvTypeFrom(IC_GRENADE, 28, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* big creature gas */ 0, 0, 0),
  createInvTypeFrom(IC_GRENADE, 29, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* very sm creat gas */ 0, 0, 0),

  //---ARMOUR---
  createInvTypeFrom(IC_ARMOUR, 0, INVALIDCURS, COND, 1, 66, 20, 0, 300, 2, /* Flak jacket     */ 0, +2, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 1, INVALIDCURS, COND, 2, 18, 22, 0, 350, 0, /* Flak jacket w X */ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 2, INVALIDCURS, COND, 2, 11, 18, 0, 400, 0, /* Flak jacket w Y */ 0, +3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 3, INVALIDCURS, COND, 1, 64, 32, 0, 500, 4, /* Kevlar jacket   */ 0, 0, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 4, INVALIDCURS, COND, 2, 16, 35, 0, 600, 0, /* Kevlar jack w X */ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 5, INVALIDCURS, COND, 2, 9, 29, 0, 700, 0, /* Kevlar jack w Y */ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 6, INVALIDCURS, COND, 1, 65, 32, 0, 1000, 8, /* Spectra jacket  */ 0, -2, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 7, INVALIDCURS, COND, 2, 17, 35, 0, 1100, 0, /* Spectra jack w X*/ 0, -3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 8, INVALIDCURS, COND, 2, 10, 29, 0, 1200, 0, /* Spectra jack w Y*/ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 9, INVALIDCURS, COND, 1, 67, 39, 0, 650, 5, /* Kevlar leggings */ 0, 0, IF_STANDARD_ARMOUR),

  createInvTypeFrom(IC_ARMOUR, 10, INVALIDCURS, COND, 2, 19, 43, 0, 800, 0, /* Kevlar legs w X */ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 11, INVALIDCURS, COND, 2, 12, 35, 0, 950, 0, /* Kevlar legs w Y */ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 12, INVALIDCURS, COND, 1, 68, 39, 0, 900, 8, /* Spectra leggings*/ 0, -2, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 13, INVALIDCURS, COND, 2, 20, 43, 0, 1100, 0, /* Spectra legs w X*/ 0, -3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 14, INVALIDCURS, COND, 2, 13, 35, 0, 1300, 0, /* Spectra legs w Y*/ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 15, INVALIDCURS, COND, 1, 61, 14, 0, 50, 2, /* Steel helmet    */ 0, +2, IF_STANDARD_ARMOUR | ITEM_METAL),
  createInvTypeFrom(IC_ARMOUR, 16, INVALIDCURS, COND, 1, 63, 14, 0, 200, 4, /* Kevlar helmet   */ 0, 0, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 17, INVALIDCURS, COND, 2, 15, 15, 0, 250, 0, /* Kevlar helm w X */ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 18, INVALIDCURS, COND, 2, 8, 13, 0, 300, 0, /* Kevlar helm w Y */ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 19, INVALIDCURS, COND, 1, 62, 14, 0, 450, 7, /* Spectra helmet  */ 0, -2, IF_STANDARD_ARMOUR),

  createInvTypeFrom(IC_ARMOUR, 20, INVALIDCURS, COND, 2, 14, 15, 0, 550, 0, /* Spectra helm w X*/ 0, -3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 21, INVALIDCURS, COND, 2, 7, 13, 0, 650, 0, /* Spectra helm w Y*/ 0, -1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 22, INVALIDCURS, COND, 1, 81, 12, 2, 250, 5, /* Ceramic plates  */ 0, -4, (IF_STANDARD_ARMOUR | ITEM_ATTACHMENT) & (~ITEM_REPAIRABLE)),
  createInvTypeFrom(IC_ARMOUR, 23, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* Infant crt hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 24, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* Yng male hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 25, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* Old male hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 26, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* Queen cret hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 27, INVALIDCURS, COND, 1, 96, 20, 0, 200, 2, /* Leather jacket  */ 0, +4, IF_STANDARD_ARMOUR),
  // NOTE: THE FOLLOWING ITEM'S PRICE VALUE IS IN DIALOGUE AND SHOULD NOT BE CHANGED
  createInvTypeFrom(IC_ARMOUR, 28, INVALIDCURS, COND, 1, 116, 20, 0, 950, 0, /* L jacket w kev  */ 0, +2, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 29, INVALIDCURS, COND, 1, 117, 20, 0, 1200, 0, /* L jacket w kev 18*/ 0, +1, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),

  createInvTypeFrom(IC_ARMOUR, 30, INVALIDCURS, COND, 1, 118, 20, 0, 1500, 0, /* L jacket w kev c*/ 0, +3, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 31, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* yng fem hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 32, INVALIDCURS, COND, 1, 0, 0, 0, 0, 0, /* old fem hide */ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE | ITEM_NOT_EDITOR | ITEM_DEFAULT_UNDROPPABLE),
  createInvTypeFrom(IC_ARMOUR, 33, INVALIDCURS, COND, 2, 25, 3, 1, 10, 1, /* t-shirt */ 0, 0, ITEM_DAMAGEABLE | ITEM_SHOW_STATUS | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_ARMOUR, 33, INVALIDCURS, COND, 2, 34, 3, 1, 10, 1, /* t-shirt D. rules*/ 0, 0, ITEM_DAMAGEABLE | ITEM_SHOW_STATUS | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_ARMOUR, 34, INVALIDCURS, COND, 1, 137, 32, 0, 700, 6, /* Kevlar2 jacket  */ 0, -1, IF_STANDARD_ARMOUR),
  createInvTypeFrom(IC_ARMOUR, 35, INVALIDCURS, COND, 2, 40, 35, 0, 800, 0, /* Kevlar2 jack w X*/ 0, -2, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_ARMOUR, 36, INVALIDCURS, COND, 2, 41, 29, 0, 900, 0, /* Kevlar2 jack w Y*/ 0, 0, IF_STANDARD_ARMOUR | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),

  //---MISC---
  createInvTypeFrom(IC_MEDKIT, 0, AIDCURS, USAGE, 1, 73, 5, 4, 100, 1, /* First aid kit */ 0, 0, IF_STANDARD_KIT),
  createInvTypeFrom(IC_MEDKIT, 0, AIDCURS, USAGE, 1, 86, 18, 0, 300, 1, /* Medical Kit   */ 0, 0, IF_STANDARD_KIT | ITEM_METAL),
  createInvTypeFrom(IC_KIT, 0, REPAIRCURS, COND, 2, 21, 50, 0, 250, 1, /* Tool Kit	     */ 0, 0, IF_STANDARD_KIT | ITEM_METAL),
  createInvTypeFrom(IC_KIT, 0, INVALIDCURS, COND, 1, 78, 3, 1, 250, 3, /* Locksmith kit */ 0, -2, IF_STANDARD_KIT | ITEM_METAL | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_KIT, 0, INVALIDCURS, COND, 1, 58, 1, 4, 250, 5, /* Camouflage kit*/ 0, 0, IF_STANDARD_KIT),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 52, 5, 4, 300, 5, /* Silencer      */ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 76, 9, 4, 500, 6, /* Sniper scope  */ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 69, 5, 2, 50, 4, /* Bipod         */ 0, +5, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 1, 77, 9, 1, 400, 7, /* Extended ear	 */ 0, -3, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),

  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 1, 74, 9, 1, 800, 7, /* Night goggles */ 0, -1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 1, 55, 2, 4, 150, 3, /* Sun goggles	 */ 0, +3, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 1, 75, 9, 1, 100, 4, /* Gas mask   	 */ 0, +1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE),
  createInvTypeFrom(IC_KIT, 0, INVALIDCURS, 0, 2, 5, 10, 4, 10, 1, /* Canteen       */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 80, 10, 1, 200, 4, /* Metal detector*/ 0, -2, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 6, 1, 4, 900, 7, /* Compound 18	 */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 42, 1, 4, 2500, 0, /* Jar/QueenBlood*/ 0, 0, ITEM_DAMAGEABLE | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 2, 1, 4, 500, 1, /* Jar/Elixir    */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MONEY, 0, INVALIDCURS, 0, 2, 1, 1, 1, 0, 0, /* Money         */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, JARCURS, COND, 1, 71, 1, 2, 10, 1, /* Glass jar		 */ 0, 0, ITEM_DAMAGEABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, COND, 1, 72, 5, 2, 50, 1, /* Jar/CreatureBlood*/ 0, 0, ITEM_DAMAGEABLE | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 70, 1, 8, 150, 4, /* Adren Booster */ 0, 0, ITEM_DAMAGEABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, COND, 1, 47, 1, 4, 100, 3, /* Detonator     */ 0, +1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ATTACHMENT | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, COND, 1, 47, 1, 4, 200, 6, /* Rem Detonator */ 0, -1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ATTACHMENT | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 93, 1, 8, 0, 0, /* Videotape     */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 94, 1, 8, 0, 0, /* Deed          */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 56, 1, 1, 0, 0, /* Letter				 */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 59, 1, 1, 0, 0, /* Diskette	  	 */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 124, 0, 1, 3000, 0, /* Chalice	     */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 120, 1, 4, 50, 0, /* Bloodcat claws*/ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 121, 1, 4, 100, 0, /* Bloodcat teeth*/ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 9, 60, 0, 400, 0, /* Bloodcat pelt */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 54, 0, 99, 0, 0, /* Switch        */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 47, 0, 99, 0, 0, /* Action item   */ 0, 0, ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 70, 1, 6, 300, 6, /* Regen Booster */ 0, 0, ITEM_DAMAGEABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 70, 0, 99, 0, 0, /* syringe 3     */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 70, 0, 99, 0, 0, /* syringe 4     */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 70, 0, 99, 0, 0, /* syringe 5     */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, COND, 1, 72, 5, 2, 10, 1, /* Jar/Human Blood*/ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 0, 0, 0, 0, 0, /* ownership     */ 0, 0, ITEM_NOT_BUYABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 51, 4, 4, 750, 8, /* Laser scope   */ 0, -1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, REMOTECURS, 0, 1, 54, 9, 4, 400, 6, /* Remote bomb trig*/ 0, -2, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, WIRECUTCURS, 0, 1, 88, 4, 2, 20, 2, /* Wirecutters   */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 90, 9, 4, 30, 2, /* Duckbill      */ 0, +5, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ATTACHMENT),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 106, 20, 1, 30, 1, /* Alcohol  */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 1, 74, 11, 1, 1500, 10, /* UV goggles */ 0, -1, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 0, 44, 21, 0, 30, 0, /* discarded LAW*/ 0, 0, IF_TWOHANDED_GUN | ITEM_NOT_BUYABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 0, 40, 0, 0, 0, /* head - generic */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 1, 40, 0, 0, 0, /* head - Imposter*/ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 2, 40, 0, 0, 0, /* head - T-Rex */ 0, 0, ITEM_DAMAGEABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 3, 40, 0, 0, 0, /* head - Slay */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 4, 40, 0, 0, 0, /* head - Druggist */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 5, 40, 0, 0, 0, /* head - Matron */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 6, 40, 0, 0, 0, /* head - Tiffany */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 100, 12, 1, 20, 1, /* wine     */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 101, 4, 4, 10, 1, /* beer		  */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 99, 0, 2, 20, 3, /* pornos   */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 0, 43, 20, 0, 900, 6, /* video camera */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 0, 42, 5, 1, 2500, 0, /* robot remote */ 0, -5, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 103, 20, 0, 500, 0, /* creature claws */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 7, 40, 0, 250, 0, /* creature flesh */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 104, 10, 0, 1000, 0, /* creature organ */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, REMOTECURS, 0, 1, 54, 9, 4, 400, 6, /* Remote trigger*/ 0, -2, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 0, 47, 2, 8, 500, 2, /* gold watch */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 136, 100, 0, 200, 2, /* golf clubs */ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL),
  createInvTypeFrom(IC_FACE, 0, INVALIDCURS, 0, 3, 11, 5, 1, 100, 1, /* walkman */ 0, -4, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 8, 50, 0, 300, 2, /* portable tv */ 0, -3, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 3, 10, 10, 1, 30, 1, /* cigars */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),

  createInvTypeFrom(IC_KEY, 0, INVALIDCURS, 0, 1, 82, 1, 8, 0, 0, /* dull gold key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 1, INVALIDCURS, 0, 1, 83, 1, 8, 0, 0, /* silver key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 2, INVALIDCURS, 0, 1, 84, 1, 8, 0, 0, /* diamond-shpd key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 3, INVALIDCURS, 0, 1, 87, 1, 8, 0, 0, /* bright gold key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 4, INVALIDCURS, 0, 1, 91, 1, 8, 0, 0, /* gold key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 5, INVALIDCURS, 0, 1, 92, 1, 8, 0, 0, /* small gold key */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 6, INVALIDCURS, 0, 1, 108, 1, 8, 0, 0, /* electronic */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_KEY, 7, INVALIDCURS, 0, 1, 109, 1, 8, 0, 0, /* passcard       */ 0, 0, ITEM_NOT_BUYABLE | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 8, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 9, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),

  createInvTypeFrom(IC_KEY, 10, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 11, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 12, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 13, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 14, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 15, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 16, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 17, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 18, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 19, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),

  createInvTypeFrom(IC_KEY, 20, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 21, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 22, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 23, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 24, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 25, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 26, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 27, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 28, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 29, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),

  createInvTypeFrom(IC_KEY, 30, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_KEY, 31, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* key      */ 0, 0, ITEM_NOT_EDITOR | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 31, 4, 0, 150, 2, /* silver platter */ 0, 0, ITEM_DAMAGEABLE | ITEM_METAL),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 30, 1, 6, 5, 1, /* duct tape */ 0, 0, ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 28, 3, 1, 20, 0, /* aluminum rod */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 32, 1, 8, 0, 0, /* spring */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 33, 4, 1, 25, 0, /* a. rod & spring */ 0, 0, ITEM_NOT_BUYABLE | ITEM_REPAIRABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_INSEPARABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 133, 4, 1, 20, 0, /* steel rod */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 132, 2, 6, 5, 3, /* quick glue */ 0, 0, ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 134, 6, 1, 150, 0, /* gun barrel xtndr */ 0, 0, ITEM_NOT_BUYABLE | ITEM_REPAIRABLE | ITEM_DAMAGEABLE | ITEM_METAL | ITEM_INSEPARABLE),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 35, 1, 8, 0, 0, /* string */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 27, 1, 1, 0, 0, /* tin can */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, TINCANCURS, 0, 2, 36, 2, 4, 0, 0, /* string & tin can */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 122, 3, 6, 5, 0, /* marbles */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 127, 6, 1, 200, 6, /* lame boy */ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 128, 1, 8, 5, 1, /* copper wire */ 0, 0, ITEM_METAL | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 126, 7, 1, 50, 0, /* display unit */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 130, 1, 1, 100, 5, /* fumble pak */ 0, 0, ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 131, 1, 2, 10, 5, /* xray bulb */ 0, 0, ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 29, 1, 8, 1, 1, /* chewing gum */ 0, 0, ITEM_DAMAGEABLE | ITEM_HIDDEN_ADDON),

  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 129, 3, 1, 100, 0, /* flash device */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC | ITEM_HIDDEN_ADDON),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 2, 26, 1, 6, 5, 1, /* batteries */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MISC, 0, INVALIDCURS, 0, 1, 123, 1, 8, 0, 0, /* elastic */ 0, 0, ITEM_NOT_BUYABLE | ITEM_UNAERODYNAMIC),
  createInvTypeFrom(IC_MISC, 0, REMOTECURS, 0, 1, 125, 10, 1, 2500, 0, /* xray device */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE | ITEM_REPAIRABLE | ITEM_METAL | ITEM_ELECTRONIC),
  createInvTypeFrom(IC_MONEY, 0, INVALIDCURS, 0, 2, 38, 2, 1, 100, 0, /* silver   */ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_MONEY, 0, INVALIDCURS, 0, 1, 135, 2, 1, 300, 0, /* gold			*/ 0, 0, ITEM_NOT_BUYABLE | ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_KIT, 0, REFUELCURS, 0, 2, 39, 20, 0, 250, 0, /* gas can */ 0, 0, ITEM_DAMAGEABLE),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
  createInvTypeFrom(IC_NONE, 0, INVALIDCURS, 0, 0, 0, 0, 0, 0, 0, /* nothing! */ 0, 0, 0),
...
export function WeaponInHand(pSoldier: SOLDIERTYPE): boolean {
  if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass & (IC_WEAPON | IC_THROWN)) {
    if (pSoldier.inv[Enum261.HANDPOS].usItem == Enum225.ROCKET_RIFLE || pSoldier.inv[Enum261.HANDPOS].usItem == Enum225.AUTO_ROCKET_RIFLE) {
      if (pSoldier.inv[Enum261.HANDPOS].ubImprintID != NO_PROFILE) {
        if (pSoldier.ubProfile != NO_PROFILE) {
          if (pSoldier.inv[Enum261.HANDPOS].ubImprintID != pSoldier.ubProfile) {
            return false;
          }
        } else {
...
function FindObjExcludingSlot(pSoldier: SOLDIERTYPE, usItem: UINT16, bExcludeSlot: INT8): INT8 {
  let bLoop: INT8;
...
export function FindObjWithin(pSoldier: SOLDIERTYPE, usItem: UINT16, bLower: INT8, bUpper: INT8): INT8 {
  let bLoop: INT8;
...
export function FindObjInObjRange(pSoldier: SOLDIERTYPE, usItem1: UINT16, usItem2: UINT16): INT8 {
  let bLoop: INT8;
  let usTemp: UINT16;
...
export function FindObjClass(pSoldier: SOLDIERTYPE, usItemClass: UINT32): INT8 {
  let bLoop: INT8;
...
function FindObjClassAfterSlot(pSoldier: SOLDIERTYPE, bStartAfter: INT8, usItemClass: UINT32): INT8 {
  let bLoop: INT8;
...
export function FindAIUsableObjClass(pSoldier: SOLDIERTYPE, usItemClass: UINT32): INT8 {
...
export function FindAIUsableObjClassWithin(pSoldier: SOLDIERTYPE, usItemClass: UINT32, bLower: INT8, bUpper: INT8): INT8 {
  let bLoop: INT8;
...
export function FindEmptySlotWithin(pSoldier: SOLDIERTYPE, bLower: INT8, bUpper: INT8): INT8 {
  let bLoop: INT8;

  for (bLoop = bLower; bLoop <= bUpper; bLoop++) {
    if (pSoldier.inv[bLoop].usItem == 0) {
      if (bLoop == Enum261.SECONDHANDPOS && Item[pSoldier.inv[Enum261.HANDPOS].usItem].fFlags & ITEM_TWO_HANDED) {
        continue;
      } else {
        return bLoop;
      }
...
export function FindAttachmentByClass(pObj: OBJECTTYPE, uiItemClass: UINT32): INT8 {
  let bLoop: INT8;
...
export function FindLaunchable(pSoldier: SOLDIERTYPE, usWeapon: UINT16): INT8 {
  let bLoop: INT8;
...
export function FindLaunchableAttachment(pObj: OBJECTTYPE, usWeapon: UINT16): INT8 {
  let bLoop: INT8;
...
export function ValidItemAttachment(pObj: OBJECTTYPE, usAttachment: UINT16, fAttemptingAttachment: boolean): boolean {
    ...
    if ((FindAttachment(pObj, Enum225.UNDER_GLAUNCHER) != ITEM_NOT_FOUND) && ValidLaunchable(usAttachment, Enum225.UNDER_GLAUNCHER)) {
      ...
      */
      ...
      if (fAttemptingAttachment && ValidAttachmentClass(usAttachment, pObj.usItem)) {
        ...
        zTemp = swprintf(Message[Enum334.STR_CANT_ATTACH], ItemNames[usAttachment], ItemNames[pObj.usItem]);
...
function ValidItemLaunchable(pObj: OBJECTTYPE, usAttachment: UINT16): boolean {
...
export function CopyObj(pSourceObj: OBJECTTYPE, pTargetObj: OBJECTTYPE): void {
...
export function GetObjFrom(pObj: OBJECTTYPE, ubGetIndex: UINT8, pDest: OBJECTTYPE): void {
...
function SwapWithinObj(pObj: OBJECTTYPE, ubIndex1: UINT8, ubIndex2: UINT8): void {
  let bTemp: INT8;
...
export function StackObjs(pSourceObj: OBJECTTYPE, pTargetObj: OBJECTTYPE, ubNumberToCopy: UINT8): void {
  let ubLoop: UINT8;
...
export function CleanUpStack(pObj: OBJECTTYPE, pCursorObj: OBJECTTYPE | null): void {
  let bLoop: INT8;
  let bLoop2: INT8;
  let bMaxPoints: INT8;
      ...
      if (pCursorObj.bStatus[bLoop] > 0) {
        ...
        for (bLoop2 = pObj.ubNumberOfObjects - 1; bLoop2 >= 0; bLoop2--) {
          if (pObj.bStatus[bLoop2] < bMaxPoints) {
            ...
            bPointsToMove = Math.min(bPointsToMove, pCursorObj.bStatus[bLoop]);
...
export function PlaceObjectAtObjectIndex(pSourceObj: OBJECTTYPE, pTargetObj: OBJECTTYPE, ubIndex: UINT8): boolean {
  let bTemp: INT8;
...
export function ReloadGun(pSoldier: SOLDIERTYPE, pGun: OBJECTTYPE, pAmmo: OBJECTTYPE): boolean {
  ...
  if (Item[pGun.usItem].usItemClass == IC_LAUNCHER || pGun.usItem == Enum225.TANK_CANNON) {
    ...
    if (fSameMagazineSize) {
      ...
      if (bReloadType == RELOAD_TOPOFF) {
        ubBulletsToMove = Math.min(pAmmo.ubShotsLeft[0], Weapon[pGun.usItem].ubMagSize - pGun.ubGunShotsLeft);
      } else {
      ...
      if (bReloadType == RELOAD_TOPOFF) {
        ...
        ubBulletsToMove = Math.min(pAmmo.ubShotsLeft[0], Weapon[pGun.usItem].ubMagSize);
      }
  ...
  if (pSoldier.bTeam == gbPlayerNum) {
    ...
    ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[Enum334.STR_PLAYER_RELOADS], pSoldier.name);
  }
...
export function EmptyWeaponMagazine(pWeapon: OBJECTTYPE, pAmmo: OBJECTTYPE): boolean {
  let usReloadSound: UINT16;
...
export function FindAmmo(pSoldier: SOLDIERTYPE, ubCalibre: UINT8, ubMagSize: UINT8, bExcludeSlot: INT8): INT8 {
  let bLoop: INT8;
  let pItem: INVTYPE;
...
export function FindAmmoToReload(pSoldier: SOLDIERTYPE, bWeaponIn: INT8, bExcludeSlot: INT8): INT8 {
  let pObj: OBJECTTYPE;
  let bSlot: INT8;
  ...
  if (Item[pObj.usItem].usItemClass == IC_GUN && pObj.usItem != Enum225.TANK_CANNON) {
      ...
      if ((fRet == true) && IsValidSecondHandShotForReloadingPurposes(pSoldier)) {
        ...
        if (bSlot != NO_SLOT) {
          ...
          if (EnoughPoints(pSoldier, bAPCost, 0, false)) {
            ...
            ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[Enum334.STR_RELOAD_ONLY_ONE_GUN], pSoldier.name);
          }
        }
      }
...
function PerformAttachmentComboMerge(pObj: OBJECTTYPE, bAttachmentComboMerge: INT8): void {
  let bAttachLoop: INT8;
  let bAttachPos: INT8;
...
export function AttachObject(pSoldier: SOLDIERTYPE | null, pTargetObj: OBJECTTYPE, pAttachment: OBJECTTYPE): boolean {
  ...
  if (fValidLaunchable || ValidItemAttachment(pTargetObj, pAttachment.usItem, true)) {
    ...
    if (fValidLaunchable || pAttachment.usItem >= Enum225.GL_HE_GRENADE && pAttachment.usItem <= Enum225.GL_SMOKE_GRENADE) {
      ...
      if (pSoldier) {
        ...
        if (ValidItemAttachment(pTargetObj, pAttachment.usItem, true)) // not launchable
        {
          ...
          if (Item[pTargetObj.usItem].usItemClass & IC_WEAPON) {
            PlayJA2Sample(Enum330.ATTACH_TO_GUN, RATE_11025, SoundVolume(MIDVOLUME, pSoldier.sGridNo), 1, SoundDir(pSoldier.sGridNo));
          } else if (Item[pTargetObj.usItem].usItemClass & IC_ARMOUR) {
            PlayJA2Sample(Enum330.ATTACH_CERAMIC_PLATES, RATE_11025, SoundVolume(MIDVOLUME, pSoldier.sGridNo), 1, SoundDir(pSoldier.sGridNo));
          } else if (Item[pTargetObj.usItem].usItemClass & IC_BOMB) {
            PlayJA2Sample(Enum330.ATTACH_DETONATOR, RATE_11025, SoundVolume(MIDVOLUME, pSoldier.sGridNo), 1, SoundDir(pSoldier.sGridNo));
          }
        }
      }
...
export function CanItemFitInPosition(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, bPos: INT8, fDoingPlacement: boolean): boolean {
  let ubSlotLimit: UINT8;
  let bNewPos: INT8;

  switch (bPos) {
    case Enum261.SECONDHANDPOS:
      if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].fFlags & ITEM_TWO_HANDED) {
        return false;
      }
      break;
    case Enum261.HANDPOS:
      if (Item[pObj.usItem].fFlags & ITEM_TWO_HANDED) {
        if (pSoldier.inv[Enum261.HANDPOS].usItem != NOTHING && pSoldier.inv[Enum261.SECONDHANDPOS].usItem != NOTHING) {
          // two items in hands; try moving the second one so we can swap
          if (Item[pSoldier.inv[Enum261.SECONDHANDPOS].usItem].ubPerPocket == 0) {
          ...
          if (fDoingPlacement) {
            // otherwise move it.
            CopyObj(pSoldier.inv[Enum261.SECONDHANDPOS], pSoldier.inv[bNewPos]);
    ...
    case Enum261.LEGPOS:
      ...
      switch (bPos) {
        case Enum261.VESTPOS:
          if (Armour[Item[pObj.usItem].ubClassIndex].ubArmourClass != Enum284.ARMOURCLASS_VEST) {
            return false;
          }
          break;
        case Enum261.HELMETPOS:
          if (Armour[Item[pObj.usItem].ubClassIndex].ubArmourClass != Enum284.ARMOURCLASS_HELMET) {
            return false;
          }
          break;
        case Enum261.LEGPOS:
          if (Armour[Item[pObj.usItem].ubClassIndex].ubArmourClass != Enum284.ARMOURCLASS_LEGGINGS) {
            return false;
          }
          break;
        default:
...
function DropObjIfThereIsRoom(pSoldier: SOLDIERTYPE, bPos: INT8, pObj: OBJECTTYPE): boolean {
...
export function PlaceObject(pSoldier: SOLDIERTYPE, bPos: INT8, pObj: OBJECTTYPE): boolean {
  ...
  if (bPos == Enum261.HEAD1POS) {
    if (!CompatibleFaceItem(pObj.usItem, pSoldier.inv[Enum261.HEAD2POS].usItem)) {
      let zTemp: string /* UINT16[150] */;

      zTemp = swprintf(Message[Enum334.STR_CANT_USE_TWO_ITEMS], ItemNames[pObj.usItem], ItemNames[pSoldier.inv[Enum261.HEAD2POS].usItem]);
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, zTemp);
      return false;
    }
  } else if (bPos == Enum261.HEAD2POS) {
    if (!CompatibleFaceItem(pObj.usItem, pSoldier.inv[Enum261.HEAD1POS].usItem)) {
      let zTemp: string /* UINT16[150] */;

      zTemp = swprintf(Message[Enum334.STR_CANT_USE_TWO_ITEMS], ItemNames[pObj.usItem], ItemNames[pSoldier.inv[Enum261.HEAD1POS].usItem]);
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_UI_FEEDBACK, zTemp);
      return false;
    }
  }
  ...
  if (pInSlot.ubNumberOfObjects == 0) {
    ...
    */
    ...
    if (pObj.ubNumberOfObjects == 0) {
      // dropped everything
      if (bPos == Enum261.HANDPOS && Item[pInSlot.usItem].fFlags & ITEM_TWO_HANDED) {
    ...
    if ((pObj.usItem == pInSlot.usItem) && (Item[pObj.usItem].usItemClass != IC_KEY || pObj.ubKeyID == pInSlot.ubKeyID)) {
      ...
      if ((Item[pObj.usItem].fFlags & ITEM_TWO_HANDED) && (bPos == Enum261.HANDPOS)) {
        if (pSoldier.inv[Enum261.SECONDHANDPOS].usItem != 0) {
...
function InternalAutoPlaceObject(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, fNewItem: boolean, bExcludeSlot: INT8): boolean {
  let bSlot: INT8;
  let pItem: INVTYPE;
  let ubPerSlot: UINT8;
  ...
  switch (pItem.usItemClass) {
    ...
    case IC_GRENADE:
      if (!(pItem.fFlags & ITEM_TWO_HANDED)) {
        ...
        } else if (!(Item[pSoldier.inv[Enum261.HANDPOS].usItem].fFlags & ITEM_TWO_HANDED) && pSoldier.inv[Enum261.SECONDHANDPOS].usItem == Enum225.NONE) {
    ...
    case IC_ARMOUR:
      ...
      break;
    case IC_FACE:
      if ((pSoldier.inv[Enum261.HEAD1POS].usItem == NOTHING) && CompatibleFaceItem(pObj.usItem, pSoldier.inv[Enum261.HEAD2POS].usItem)) {
        PlaceObject(pSoldier, Enum261.HEAD1POS, pObj);
        SetNewItem(pSoldier, Enum261.HEAD1POS, fNewItem);
        if (pObj.ubNumberOfObjects == 0) {
          return true;
        }
      } else if ((pSoldier.inv[Enum261.HEAD2POS].usItem == NOTHING) && CompatibleFaceItem(pObj.usItem, pSoldier.inv[Enum261.HEAD1POS].usItem)) {
  ...
  if (ubPerSlot == 0) {
    ...
    if (ubPerSlot > 1 || Item[pObj.usItem].usItemClass == IC_KEY || Item[pObj.usItem].usItemClass == IC_MONEY) {
      ...
      bSlot = Enum261.HANDPOS;
      while (1) {
        bSlot = FindObjWithin(pSoldier, pObj.usItem, bSlot, Enum261.SMALLPOCK8POS);
        if (bSlot == ITEM_NOT_FOUND) {
          break;
        }
...
export function AutoPlaceObject(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, fNewItem: boolean): boolean {
  return InternalAutoPlaceObject(pSoldier, pObj, fNewItem, NO_SLOT);
}

export function RemoveObjectFromSlot(pSoldier: SOLDIERTYPE, bPos: INT8, pObj: OBJECTTYPE): boolean {
  if (!pObj) {
    return false;
  }
...
export function RemoveKeyFromSlot(pSoldier: SOLDIERTYPE, bKeyRingPosition: INT8, pObj: OBJECTTYPE): boolean {
  let ubItem: UINT8 = 0;
...
export function RemoveKeysFromSlot(pSoldier: SOLDIERTYPE, bKeyRingPosition: INT8, ubNumberOfKeys: UINT8, pObj: OBJECTTYPE): boolean {
  let ubItems: UINT8 = 0;
...
// return number added
export function AddKeysToSlot(pSoldier: SOLDIERTYPE, bKeyRingPosition: INT8, pObj: OBJECTTYPE): UINT8 {
...
export function SwapKeysToSlot(pSoldier: SOLDIERTYPE, bKeyRingPosition: INT8, pObj: OBJECTTYPE): UINT8 {
...
export function CreateKeyObject(pObj: OBJECTTYPE, ubNumberOfKeys: UINT8, ubKeyID: UINT8): boolean {
  let fRet: boolean;
...
export function UseKitPoints(pObj: OBJECTTYPE, usPoints: UINT16, pSoldier: SOLDIERTYPE): UINT16 {
...
export function RandomMagazine(usItem: UINT16, ubPercentStandard: UINT8): UINT16 {
  ...
  let pWeapon: WEAPONTYPE;
  let usLoop: UINT16;
  let usPossibleMagIndex: UINT16[] /* [MAX_AMMO_TYPES_PER_GUN] */ = createArray(MAX_AMMO_TYPES_PER_GUN, 0);
...
export function CreateItem(usItem: UINT16, bStatus: INT8, pObj: OBJECTTYPE): boolean {
  let fRet: boolean;
...
export function CreateItems(usItem: UINT16, bStatus: INT8, ubNumber: UINT8, pObj: OBJECTTYPE): boolean {
  let fOk: boolean;
  let ubLoop: UINT8;
...
export function ArmBomb(pObj: OBJECTTYPE, bSetting: INT8): boolean {
  ...
  if (pObj.usItem == Enum225.ACTION_ITEM) {
    ...
    fTimed = true;
  } else if ((FindAttachment(pObj, Enum225.REMDETONATOR) != ITEM_NOT_FOUND) || (pObj.usItem == Enum225.ACTION_ITEM)) {
    fRemote = true;
  } else if (pObj.usItem == Enum225.MINE || pObj.usItem == Enum225.TRIP_FLARE || pObj.usItem == Enum225.TRIP_KLAXON || pObj.usItem == Enum225.ACTION_ITEM) {
    fPressure = true;
...
export function SetNewItem(pSoldier: SOLDIERTYPE, ubInvPos: UINT8, fNewItem: boolean): void {
  if (fNewItem) {
...
export function PlaceObjectInSoldierProfile(ubProfile: UINT8, pObject: OBJECTTYPE): boolean {
  let bLoop: INT8;
  let bLoop2: INT8;
...
function DamageItem(pObject: OBJECTTYPE, iDamage: INT32, fOnGround: boolean): boolean {
  let bLoop: INT8;
  let bDamage: INT8;
...
export function CheckEquipmentForDamage(pSoldier: SOLDIERTYPE, iDamage: INT32): void {
  let bSlot: INT8;
...
export function CheckEquipmentForFragileItemDamage(pSoldier: SOLDIERTYPE, iDamage: INT32): void {
    ...
    switch (pSoldier.inv[bSlot].usItem) {
      ...
      case Enum225.JAR_ELIXIR:
        ...
        if (!fPlayedGlassBreak && (ubNumberOfObjects != pSoldier.inv[bSlot].ubNumberOfObjects)) {
          PlayJA2Sample(Enum330.GLASS_CRACK, RATE_11025, SoundVolume(MIDVOLUME, pSoldier.sGridNo), 1, SoundDir(pSoldier.sGridNo));
...
export function DamageItemOnGround(pObject: OBJECTTYPE, sGridNo: INT16, bLevel: INT8, iDamage: INT32, ubOwner: UINT8): boolean {
  let fBlowsUp: boolean;
...
export function SwapHandItems(pSoldier: SOLDIERTYPE): void {
  ...
  if (pSoldier.inv[Enum261.HANDPOS].usItem == NOTHING || pSoldier.inv[Enum261.SECONDHANDPOS].usItem == NOTHING) {
    ...
    if (TwoHandedItem(pSoldier.inv[Enum261.SECONDHANDPOS].usItem)) {
      ...
      fOk = InternalAutoPlaceObject(pSoldier, pSoldier.inv[Enum261.HANDPOS], false, Enum261.HANDPOS);
      if (!fOk) {
        return;
      }
...
export function WaterDamage(pSoldier: SOLDIERTYPE): void {
  ...
  if (pSoldier.bCamo > 0 && !HAS_SKILL_TRAIT(pSoldier, Enum269.CAMOUFLAGED)) {
    ...
    if (pSoldier.bCamo == 0) {
      ...
      ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[Enum334.STR_CAMMO_WASHED_OFF], pSoldier.name);
    }
  }
...
export function ApplyCammo(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, pfGoodAPs: Pointer<boolean>): boolean {
...
export function ApplyCanteen(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, pfGoodAPs: Pointer<boolean>): boolean {
...
export function ApplyElixir(pSoldier: SOLDIERTYPE, pObj: OBJECTTYPE, pfGoodAPs: Pointer<boolean>): boolean {
  let sPointsToUse: INT16;
  let usTotalKitPoints: UINT16;
...
export function ActivateXRayDevice(pSoldier: SOLDIERTYPE): void {
  ...
  // check for batteries
  bBatteries = FindAttachment(pSoldier.inv[Enum261.HANDPOS], Enum225.BATTERIES);
