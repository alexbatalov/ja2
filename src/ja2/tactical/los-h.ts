...
export const INT32_TO_FIXEDPT = (n) => ((n) << FIXEDPT_FRACTIONAL_BITS);
export const FIXEDPT_TO_INT32 = (n) => ((n) / FIXEDPT_FRACTIONAL_RESOLUTION);

export const FIXEDPT_TO_TILE_NUM = (n) => (FIXEDPT_TO_INT32((n)) / CELL_X_SIZE);
export const FIXEDPT_TO_LOS_INDEX = (n) => (CONVERT_WITHINTILE_TO_INDEX(FIXEDPT_TO_INT32((n)) % CELL_X_SIZE));
...
export const CONVERT_PIXELS_TO_HEIGHTUNITS = (n) => ((n) * HEIGHT_UNITS / MAX_STRUCTURE_HEIGHT);
const CONVERT_PIXELS_TO_INDEX = (n) => ((n) * HEIGHT_UNITS / MAX_STRUCTURE_HEIGHT / HEIGHT_UNITS_PER_INDEX);
export const CONVERT_HEIGHTUNITS_TO_INDEX = (n) => ((n) / HEIGHT_UNITS_PER_INDEX);
export const CONVERT_HEIGHTUNITS_TO_DISTANCE = (n) => ((n) / (HEIGHT_UNITS / CELL_X_SIZE));
export const CONVERT_HEIGHTUNITS_TO_PIXELS = (n) => ((n) * MAX_STRUCTURE_HEIGHT / HEIGHT_UNITS);
export const CONVERT_WITHINTILE_TO_INDEX = (n) => ((n) >> 1);
const CONVERT_INDEX_TO_WITHINTILE = (n) => ((n) << 1);
export const CONVERT_INDEX_TO_PIXELS = (n) => ((n) * MAX_STRUCTURE_HEIGHT * HEIGHT_UNITS_PER_INDEX / HEIGHT_UNITS);
