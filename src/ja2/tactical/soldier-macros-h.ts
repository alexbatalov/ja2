...
const AM_A_ROBOT = (p) => ((p.value.ubProfile == NO_PROFILE) ? FALSE : (gMercProfiles[p.value.ubProfile].ubBodyType == ROBOTNOWEAPON));
...
const OK_CONTROLLABLE_MERC = (p) => (p.value.bLife >= OKLIFE && p.value.bActive && p.value.bInSector && p.value.bTeam == gbPlayerNum && p.value.bAssignment < ON_DUTY);

// Checks if our guy can be controllable .... checks bInSector, team, on duty, etc...
const OK_INSECTOR_MERC = (p) => (p.value.bLife >= OKLIFE && p.value.bActive && p.value.bInSector && p.value.bTeam == gbPlayerNum && p.value.bAssignment < ON_DUTY);
...
const CREATURE_OR_BLOODCAT = (p) => ((p.value.uiStatusFlags & SOLDIER_MONSTER) || p.value.ubBodyType == BLOODCAT);

const TANK = (p) => (p.value.ubBodyType == TANK_NE || p.value.ubBodyType == TANK_NW);
