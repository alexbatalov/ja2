    ...
    case Enum171.SCHEDULE_ACTION_LEAVESECTOR:
      ...
      switch (pSoldier.bAIScheduleProgress) {
        ...
        case 1: // near edge
          ...
          pSoldier.usActionData = FindNearbyPointOnEdgeOfMap(pSoldier, bDirection__Pointer);
          if (pSoldier.usActionData == NOWHERE) {
            // what the heck??
            // ABORT!
            DoneScheduleAction(pSoldier);
...
function DecideActionNamedNPC(pSoldier: SOLDIERTYPE): INT8 {
  ...
  let ubDesiredMerc__Pointer = createPointer(() => ubDesiredMerc, (v) => ubDesiredMerc = v);
  let sDesiredMercDist: INT16 = 0;
  let sDesiredMercDist__Pointer = createPointer(() => sDesiredMercDist, (v) => sDesiredMercDist = v);
  ...
  if (pSoldier.ubQuoteRecord && pSoldier.ubQuoteActionID == Enum290.QUOTE_ACTION_ID_TURNTOWARDSPLAYER) {
    ...
    ///////////////
    pSoldier.usActionData = NPCConsiderInitiatingConv(pSoldier, ubDesiredMerc__Pointer);
    if (pSoldier.usActionData != NOWHERE) {
    ...
    case Enum268.TYRONE:
      ...
      if (sDesiredMercLoc != NOWHERE) {
        if (sDesiredMercDist <= NPC_TALK_RADIUS * 2) {
          ...
          pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sDesiredMercLoc, Enum289.AI_ACTION_APPROACH_MERC);
          if (pSoldier.usActionData != NOWHERE) {
            return Enum289.AI_ACTION_APPROACH_MERC;
          }
        }
...
function DecideActionGreen(pSoldier: SOLDIERTYPE): INT8 {
  ...
  let fCivilian: boolean = (PTR_CIVILIAN(pSoldier) && (pSoldier.ubCivilianGroup == Enum246.NON_CIV_GROUP || pSoldier.bNeutral || (pSoldier.ubBodyType >= Enum194.FATCIV && pSoldier.ubBodyType <= Enum194.CRIPPLECIV)));
  ...
  if (!gubNPCPathCount) // try to limit pathing in Green AI
  {
    ...
    if (bInWater || (PreRandom(100) < iChance)) {
      ...
      if (pSoldier.usActionData != NOWHERE) {
        pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, pSoldier.usActionData, Enum289.AI_ACTION_RANDOM_PATROL);
      }
  ...
  if (!gubNPCPathCount) // try to limit pathing in Green AI
  {
    ...
    if (PreRandom(100) < iChance) {
      if (RandomFriendWithin(pSoldier)) {
        if (pSoldier.usActionData == GoAsFarAsPossibleTowards(pSoldier, pSoldier.usActionData, Enum289.AI_ACTION_SEEK_FRIEND)) {
  ...
  if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier.bActionPoints) {
    ...
    if (pSoldier.bLastAction != Enum289.AI_ACTION_CHANGE_FACING) {
      ...
      // set base chance according to orders
      if (pSoldier.bOrders == Enum241.STATIONARY)
        iChance += 25;

      if (pSoldier.bOrders == Enum241.ONGUARD)
        iChance += 20;

      if (pSoldier.bAttitude == Enum242.DEFENSIVE)
        iChance += 25;
...
function DecideActionYellow(pSoldier: SOLDIERTYPE): INT8 {
  ...
  let iNoiseValue__Pointer = createPointer(() => iNoiseValue, (v) => iNoiseValue = v);
  let iChance: INT32;
  let iSneaky: INT32;
  let sClosestFriend: INT16;
  let fCivilian: boolean = (PTR_CIVILIAN(pSoldier) && (pSoldier.ubCivilianGroup == Enum246.NON_CIV_GROUP || pSoldier.bNeutral || (pSoldier.ubBodyType >= Enum194.FATCIV && pSoldier.ubBodyType <= Enum194.CRIPPLECIV)));
  let fClimb: boolean = false;
  let fClimb__Pointer = createPointer(() => fClimb, (v) => fClimb = v);
  let fReachable: boolean = false;
  let fReachable__Pointer = createPointer(() => fReachable, (v) => fReachable = v);
  ...
  sNoiseGridNo = MostImportantNoiseHeard(pSoldier, iNoiseValue__Pointer, fClimb__Pointer, fReachable__Pointer);
  ...
  if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier.bActionPoints) {
    if ((pSoldier.bDirection != ubNoiseDir) && PythSpacesAway(pSoldier.sGridNo, sNoiseGridNo) <= MaxDistanceVisible()) {
      // set base chance according to orders
      if ((pSoldier.bOrders == Enum241.STATIONARY) || (pSoldier.bOrders == Enum241.ONGUARD))
        iChance = 60;
  ...
  if (!(pSoldier.bTeam == CIV_TEAM && pSoldier.ubProfile != NO_PROFILE && pSoldier.ubProfile != Enum268.ELDIN)) {
    ...
    if (fReachable) {
      ...
      if (PreRandom(100) < iChance) {
        pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sNoiseGridNo, Enum289.AI_ACTION_SEEK_NOISE);
    ...
    sClosestFriend = ClosestReachableFriendInTrouble(pSoldier, fClimb__Pointer);
    ...
    if (sClosestFriend != NOWHERE) {
      ...
      if (PreRandom(100) < iChance) {
        pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sClosestFriend, Enum289.AI_ACTION_SEEK_FRIEND);
    ...
    if (!SkipCoverCheck && gfTurnBasedAI) // only do in turnbased
    {
      ...
      if (PreRandom(100) < iChance) {
        ...
        pSoldier.usActionData = FindBestNearbyCover(pSoldier, pSoldier.bAIMorale, iDummy__Pointer);
  ...
  if (!fCivilian && !PTR_CROUCHED(pSoldier) && IsValidStance(pSoldier, ANIM_CROUCH)) {
...
export function DecideActionRed(pSoldier: SOLDIERTYPE, ubUnconsciousOK: UINT8): INT8 {
  ...
  let fClimb: boolean = false;
  let fClimb__Pointer = createPointer(() => fClimb, (v) => fClimb = v);
  let fCivilian: boolean = (PTR_CIVILIAN(pSoldier) && (pSoldier.ubCivilianGroup == Enum246.NON_CIV_GROUP || (pSoldier.bNeutral && gTacticalStatus.fCivGroupHostile[pSoldier.ubCivilianGroup] == CIV_GROUP_NEUTRAL) || (pSoldier.ubBodyType >= Enum194.FATCIV && pSoldier.ubBodyType <= Enum194.CRIPPLECIV)));
  ...
  if ((!PTR_CIVILIAN(pSoldier) || pSoldier.ubProfile == Enum268.WARDEN) && ((gTacticalStatus.Team[pSoldier.bTeam].bAwareOfOpposition || (pSoldier.ubID == gTacticalStatus.ubTheChosenOne) || (pSoldier.ubProfile == Enum268.WARDEN)) && (gTacticalStatus.fPanicFlags & (PANIC_BOMBS_HERE | PANIC_TRIGGERS_HERE)))) {
    if (pSoldier.ubProfile == Enum268.WARDEN && gTacticalStatus.ubTheChosenOne == NOBODY) {
  ...
  if (pSoldier.ubProfile != NO_PROFILE) {
    if ((pSoldier.ubProfile == Enum268.QUEEN || pSoldier.ubProfile == Enum268.JOE) && ubCanMove) {
  ...
  if (fCivilian && !(pSoldier.ubBodyType == Enum194.COW || pSoldier.ubBodyType == Enum194.CRIPPLECIV)) {
  ...
  ////////////////////////////////////////////////////////////////////////////
  if (ubCanMove && InLightAtNight(pSoldier.sGridNo, pSoldier.bLevel) && pSoldier.bOrders != Enum241.STATIONARY) {
  ...
  if (gfTurnBasedAI && !fCivilian && !bInWater && !bInGas && !(pSoldier.uiStatusFlags & SOLDIER_BOXER) && (CanNPCAttack(pSoldier) == 1)) {
    ...
    if (ubCanMove && pSoldier.bActionPoints > MAX_AP_CARRIED && !fCivilian) {
      ...
      while ((bSeekPts > -90) || (bHelpPts > -90) || (bHidePts > -90)) {
        ...
        if ((bSeekPts > -90) && (bSeekPts >= bHelpPts) && (bSeekPts >= bHidePts) && (bSeekPts >= bWatchPts)) {
          ...
          sClosestDisturbance = ClosestReachableDisturbance(pSoldier, ubUnconsciousOK, fClimb__Pointer);
          ...
          if (sClosestDisturbance != NOWHERE) {
            ...
            pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sClosestDisturbance, Enum289.AI_ACTION_SEEK_OPPONENT);
              ...
              switch (pSoldier.bAttitude) {
                ...
                case Enum242.CUNNINGAID:
                  if (PythSpacesAway(pSoldier.usActionData, sClosestDisturbance) < 5 || LocationToLocationLineOfSightTest(pSoldier.usActionData, pSoldier.bLevel, sClosestDisturbance, pSoldier.bLevel, MaxDistanceVisible(), true)) {
                    ...
                    pSoldier.usActionData = InternalGoAsFarAsPossibleTowards(pSoldier, sClosestDisturbance, (MinAPsToAttack(pSoldier, sClosestDisturbance, ADDTURNCOST) + AP_CROUCH), Enum289.AI_ACTION_SEEK_OPPONENT, FLAG_CAUTIOUS);
                    if (pSoldier.usActionData != NOWHERE) {
        ...
        if ((bHelpPts > -90) && (bHelpPts >= bSeekPts) && (bHelpPts >= bHidePts) && (bHelpPts >= bWatchPts)) {
          ...
          if (sClosestFriend != NOWHERE) {
            ...
            //////////////////////////////////////////////////////////////////////
            pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sClosestFriend, Enum289.AI_ACTION_SEEK_OPPONENT);
          ...
          if (!SkipCoverCheck && sClosestOpponent != NOWHERE) {
            ...
            if (pSoldier.usActionData != NOWHERE) {
              ...
              if (sClosestDisturbance != NOWHERE && (SpacesAway(pSoldier.usActionData, sClosestDisturbance) < 5 || SpacesAway(pSoldier.usActionData, sClosestDisturbance) + 5 < SpacesAway(pSoldier.sGridNo, sClosestDisturbance))) {
                ...
                if (InternalGoAsFarAsPossibleTowards(pSoldier, pSoldier.usActionData, (MinAPsToAttack(pSoldier, sClosestOpponent, ADDTURNCOST) + AP_CROUCH), Enum289.AI_ACTION_TAKE_COVER, 0) == pSoldier.usActionData) {
                  return Enum289.AI_ACTION_TAKE_COVER;
                }
              } else {
                return Enum289.AI_ACTION_TAKE_COVER;
    ...
    if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier.bActionPoints) {
      ...
      if ((pSoldier.bDirection != ubOpponentDir)) {
        if ((pSoldier.bActionPoints == pSoldier.bInitialActionPoints || PreRandom(100) < 60) && InternalIsValidStance(pSoldier, ubOpponentDir, gAnimControl[pSoldier.usAnimState].ubEndHeight)) {
          ...
          if (pSoldier.bLastAction == Enum289.AI_ACTION_CHANGE_FACING && PreRandom(100) < 60) {
            if (gfTurnBasedAI) {
              pSoldier.bNextAction = Enum289.AI_ACTION_END_TURN;
            } else {
              pSoldier.bNextAction = Enum289.AI_ACTION_WAIT;
  ...
  if (ubCanMove && !pSoldier.bNeutral && (gfTurnBasedAI || pSoldier.bTeam == ENEMY_TEAM)) {
    pSoldier.bAction = SearchForItems(pSoldier, Enum293.SEARCH_GENERAL_ITEMS, pSoldier.inv[Enum261.HANDPOS].usItem);
  ...
  if ((pSoldier.bAlertStatus != Enum243.STATUS_BLACK) && !pSoldier.bUnderFire && ubCanMove && (!gfTurnBasedAI || pSoldier.bActionPoints >= pSoldier.bInitialActionPoints) && (ClosestReachableDisturbance(pSoldier, 1, fClimb__Pointer) == NOWHERE)) {
    ...
    if (pSoldier.bBleeding > MIN_BLEEDING_THRESHOLD) {
      ...
      pSoldier.bBleeding = Math.max(0, pSoldier.bBleeding - pSoldier.bActionPoints);
...
function DecideActionBlack(pSoldier: SOLDIERTYPE): INT8 {
  ...
  if ((pSoldier.bTeam == ENEMY_TEAM || pSoldier.ubProfile == Enum268.WARDEN) && (gTacticalStatus.fPanicFlags & PANIC_TRIGGERS_HERE) && (gTacticalStatus.ubTheChosenOne == NOBODY)) {
  ...
  if ((pSoldier.bActionPoints < 2) || bInDeepWater || bInGas || pSoldier.bRTPCombat == RTP_COMBAT_REFRAIN) {
    ...
    do {
      ...
      if (bCanAttack != 1) {
        if (fCivilian) {
          if ((bCanAttack == NOSHOOT_NOWEAPON) && !(pSoldier.uiStatusFlags & SOLDIER_BOXER) && pSoldier.ubBodyType != Enum194.COW && pSoldier.ubBodyType != Enum194.CRIPPLECIV) {
            // cower in fear!!
            if (pSoldier.uiStatusFlags & SOLDIER_COWERING) {
              if (pSoldier.bLastAction == Enum289.AI_ACTION_COWER) {
                // do nothing
          ...
          pSoldier.bAction = SearchForItems(pSoldier, Enum293.SEARCH_AMMO, pSoldier.inv[Enum261.HANDPOS].usItem);
  ...
  if (FindAIUsableObjClass(pSoldier, IC_GUN) == ITEM_NOT_FOUND && ubCanMove && !pSoldier.bNeutral) {
    ...
    pSoldier.bAction = SearchForItems(pSoldier, Enum293.SEARCH_WEAPONS, pSoldier.inv[Enum261.HANDPOS].usItem);
    if (pSoldier.bAction != Enum289.AI_ACTION_NONE) {
      return pSoldier.bAction;
    }
  }
  ...
  if (bCanAttack) {
    ...
    if (bWeaponIn != NO_SLOT) {
      ...
      if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass == IC_GUN && pSoldier.inv[Enum261.HANDPOS].bGunStatus >= USABLE) {
        ...
        if (pSoldier.bActionPoints >= ubMinAPCost) {
          ...
          if (BestShot.ubPossible) {
            ...
            if ((Menptr[BestShot.ubOpponent].bLife < OKLIFE) && !Menptr[BestShot.ubOpponent].bService) {
              ...
              if (pSoldier.bAttitude != Enum242.AGGRESSIVE || BestShot.ubChanceToReallyHit < 60) {
                // get the location of the closest CONSCIOUS reachable opponent
                sClosestDisturbance = ClosestReachableDisturbance(pSoldier, 0, fClimb__Pointer);
    ...
    if (bWeaponIn != NO_SLOT) {
      ...
      if (pSoldier.bActionPoints >= ubMinAPCost) {
        ...
        if (Item[pSoldier.inv[Enum261.HANDPOS].usItem].usItemClass & IC_THROWING_KNIFE) {
  ...
  if (pSoldier.bActionPoints == pSoldier.bInitialActionPoints && ubBestAttackAction == Enum289.AI_ACTION_FIRE_GUN && (pSoldier.bShock == 0) && (pSoldier.bLife >= pSoldier.bLifeMax / 2) && BestAttack.ubChanceToReallyHit < 30 && (PythSpacesAway(pSoldier.sGridNo, BestAttack.sTarget) > Weapon[pSoldier.inv[BestAttack.bWeaponIn].usItem].usRange / CELL_X_SIZE) && RangeChangeDesire(pSoldier) >= 4) {
    ...
    if (ubCanMove && pSoldier.bOrders != Enum241.STATIONARY && !gfHiddenInterrupt && !(pSoldier.uiStatusFlags & SOLDIER_BOXER)) {
      ...
      if (((pSoldier.bTeam == MILITIA_TEAM) && (PreRandom(20) > BestAttack.ubChanceToReallyHit)) || ((pSoldier.bTeam != MILITIA_TEAM) && (PreRandom(40) > BestAttack.ubChanceToReallyHit))) {
  ...
  if ((ubCanMove && !SkipCoverCheck && !gfHiddenInterrupt && ((ubBestAttackAction == Enum289.AI_ACTION_NONE) || pSoldier.bLastAttackHit) && (pSoldier.bTeam != gbPlayerNum || pSoldier.fAIFlags & AI_RTP_OPTION_CAN_SEEK_COVER) && !(pSoldier.uiStatusFlags & SOLDIER_BOXER)) || fAllowCoverCheck) {
    sBestCover = FindBestNearbyCover(pSoldier, pSoldier.bAIMorale, createPointer(() => iCoverPercentBetter, (v) => iCoverPercentBetter = v));
  }
  ...
  if ((ubBestAttackAction != Enum289.AI_ACTION_NONE) && (sBestCover != NOWHERE)) {
    ...
    if (pSoldier.bOrders == Enum241.SEEKENEMY)
      iOffense += 10;
    else if ((pSoldier.bOrders == Enum241.STATIONARY) || (pSoldier.bOrders == Enum241.ONGUARD))
      iDefense += 10;
  ...
  if (ubBestAttackAction != Enum289.AI_ACTION_NONE) {
    ...
    if (ubBestAttackAction == Enum289.AI_ACTION_FIRE_GUN) {
      ...
      if (IsGunBurstCapable(pSoldier, BestAttack.bWeaponIn, false) && !(Menptr[BestShot.ubOpponent].bLife < OKLIFE) && // don't burst at downed targets
          pSoldier.inv[BestAttack.bWeaponIn].ubGunShotsLeft > 1 && (pSoldier.bTeam != gbPlayerNum || pSoldier.bRTPCombat == RTP_COMBAT_AGGRESSIVE)) {
        ...
        if (pSoldier.bActionPoints - (BestAttack.ubAPCost - BestAttack.ubAimTime) >= ubBurstAPs) {
          ...
          if (PreRandom(100) < iChance) {
            ...
            BestAttack.ubAPCost = BestAttack.ubAPCost - BestAttack.ubAimTime + CalcAPsToBurst(CalcActionPoints(pSoldier), pSoldier.inv[Enum261.HANDPOS]);
      ...
      */
      ...
      if (BestAttack.bWeaponIn != Enum261.HANDPOS && gAnimControl[pSoldier.usAnimState].ubEndHeight == ANIM_STAND) {
  ...
  if (pSoldier.uiStatusFlags & SOLDIER_BOXER) {
    if (ubCanMove) {
      ...
      if (sClosestOpponent != NOWHERE) {
        ...
        pSoldier.usActionData = GoAsFarAsPossibleTowards(pSoldier, sClosestOpponent, Enum289.AI_ACTION_GET_CLOSER);
  ...
  if ((gTacticalStatus.ubSpottersCalledForBy != NOBODY) && (pSoldier.bActionPoints >= AP_RADIO) && (pSoldier.bOppCnt > 1) && !fCivilian && (gTacticalStatus.Team[pSoldier.bTeam].bMenInSector > 1) && !bInDeepWater) {
    // base chance depends on how much new info we have to radio to the others
    ...
    if (!fCivilian && !gfHiddenInterrupt && IsValidStance(pSoldier, ANIM_CROUCH)) {
      ...
      if (pSoldier.usActionData != 0) {
        if (pSoldier.usActionData == ANIM_PRONE) {
          ...
          if ((!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier.bActionPoints - GetAPsToChangeStance(pSoldier, pSoldier.usActionData)) && (((pSoldier.bAIMorale > Enum244.MORALE_HOPELESS) || ubCanMove) && !AimingGun(pSoldier))) {
            // determine the location of the known closest opponent
            // (don't care if he's conscious, don't care if he's reachable at all)
  ...
  if (!gfTurnBasedAI || GetAPsToLook(pSoldier) <= pSoldier.bActionPoints) {
    ...
    if (!gfHiddenInterrupt && ((pSoldier.bAIMorale > Enum244.MORALE_HOPELESS) || ubCanMove) && !AimingGun(pSoldier)) {
      // determine the location of the known closest opponent
      // (don't care if he's conscious, don't care if he's reachable at all)
...
export function DecideAction(pSoldier: SOLDIERTYPE): INT8 {
  ...
  } else // "normal" NPC AI
  {
    ...
    if ((pSoldier.bBypassToGreen) && (pSoldier.bAlertStatus < Enum243.STATUS_BLACK)) {
...
export function DecideAlertStatus(pSoldier: SOLDIERTYPE): void {
    ...
    } else // no opponents are in sight
    {
      switch (bOldStatus) {
        ...
        case Enum243.STATUS_YELLOW:
          ...
          if (!PTR_CIVILIAN(pSoldier) && (gTacticalStatus.Team[pSoldier.bTeam].bAwareOfOpposition || pSoldier.bUnderFire)) {
            pSoldier.bAlertStatus = Enum243.STATUS_RED;
          } else {
  ...
  if (gTacticalStatus.bBoxingState == Enum247.NOT_BOXING) {
    ...
    if (pSoldier.bAlertStatus != bOldStatus) {
      ...
      if ((bOldStatus < Enum243.STATUS_RED) || (pSoldier.bAlertStatus < Enum243.STATUS_RED)) {
    ...
    } else // status didn't change
    {
      ...
      if (((pSoldier.bAlertStatus == Enum243.STATUS_GREEN) && (pSoldier.bBreath < 75)) || ((pSoldier.bAlertStatus == Enum243.STATUS_YELLOW) && (pSoldier.bBreath < 50))) {
