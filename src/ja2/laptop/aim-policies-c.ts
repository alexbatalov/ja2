...
const AIM_POLICY_TITLE_STATEMENT_X = IMAGE_OFFSET_X + Math.trunc((500 - AIM_POLICY_TITLE_STATEMENT_WIDTH) / 2) + 5; // 80
...
function DisplayAimPolicyParagraph(usPosY: UINT16, ubPageNum: UINT8, fNumber: FLOAT): UINT16 {
  ...
  if (fNumber != 0.0) {
    ...
    sTemp = swprintf("%s", fNumber.toFixed(1).padStart(2));
...
function DisplayAimPolicySubParagraph(usPosY: UINT16, ubPageNum: UINT8, fNumber: FLOAT): UINT16 {
  ...
  // Display the section number
  sTemp = swprintf("%s", fNumber.toFixed(2).padStart(2));
