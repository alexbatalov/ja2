...
function EnterLaptop(): boolean {
  ...
  if (!(guiLAPTOP = AddVideoObject(VObjectDesc))) {
    return false;
  }
  ...
  if (!(guiLaptopBACKGROUND = AddVideoObject(VObjectDesc))) {
    return false;
  }
  ...
  if (!(guiTITLEBARLAPTOP = AddVideoObject(VObjectDesc))) {
    return false;
  }
  ...
  if (!(guiLIGHTS = AddVideoObject(VObjectDesc))) {
    return false;
  }
  ...
  if (!(guiTITLEBARICONS = AddVideoObject(VObjectDesc))) {
    return false;
  }
  ...
  if (!(guiEmailWarning = AddVideoObject(VObjectDesc))) {
    return false;
  }
  // load background
...
function LoadLoadPending(): boolean {
  ...
  if (!(guiGRAPHWINDOW = AddVideoObject(VObjectDesc))) {
    return false;
  }
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  VObjectDesc.ImageFile = FilenameForBPP("LAPTOP\\graphsegment.sti");
  if (!(guiGRAPHBAR = AddVideoObject(VObjectDesc))) {
    return false;
  }
...
function DisplayLoadPending(): boolean {
  ...
  hLapTopIconHandle = GetVideoObject(guiGRAPHWINDOW);
...
function DisplayErrorBox(): void {
  ...
  // the bottom
  hLapTopIconHandle = GetVideoObject(guiBOOKBOT);
...
export function WebPageTileBackground(ubNumX: UINT8, ubNumY: UINT8, usWidth: UINT16, usHeight: UINT16, uiBackgroundIdentifier: UINT32): boolean {
  ...
  hBackGroundHandle = GetVideoObject(uiBackgroundIdentifier);
...
function LoadDesktopBackground(): boolean {
  ...
  if ((guiDESKTOP = AddVideoSurface(vs_desc)) === -1) {
    return false;
  }
...
function DisplayTaskBarIcons(): void {
  ...
  hPixHandle = GetVideoObject(guiTITLEBARICONS);
...
export function RenderWWWProgramTitleBar(): boolean {
  ...
  VObjectDesc.fCreateFlags = VOBJECT_CREATE_FROMFILE;
  VObjectDesc.ImageFile = FilenameForBPP("LAPTOP\\programtitlebar.sti");
  if (!(uiTITLEFORWWW = AddVideoObject(VObjectDesc))) {
    return false;
  }

  // blit title
  hHandle = GetVideoObject(uiTITLEFORWWW);
