...
function BtnBobbyRPageMenuCallback(btn: GUI_BUTTON, reason: INT32): void {
...
function BtnBobbyRNextPreviousPageCallback(btn: GUI_BUTTON, reason: INT32): void {
  let bNewValue: UINT32;
...
function DisplayCostAndQty(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16, usBobbyIndex: UINT16, fUsed: boolean): UINT16 {
  ...
  */
  ...
  sTemp = swprintf("%s", (GetWeightBasedOnMetricOption(Item[usIndex].ubWeight) / (10.0)).toFixed(2).padStart(3));
  ...
  if (fUsed)
    sTemp = swprintf("%s", LaptopSaveInfo.BobbyRayUsedInventory[usBobbyIndex].ubQtyOnHand.toString().padStart(4));
  else
    sTemp = swprintf("%s", LaptopSaveInfo.BobbyRayInventory[usBobbyIndex].ubQtyOnHand.toString().padStart(4));
...
function DisplayRof(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  if (WeaponROF[usIndex] == -1)
    sTemp = swprintf("? %s", pMessageStrings[Enum333.MSG_RPM]);
  else
    sTemp = swprintf("%s/%s", WeaponROF[usIndex].toString().padStart(3), pMessageStrings[Enum333.MSG_MINUTE_ABBREVIATION]);
...
function DisplayDamage(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  sTemp = swprintf("%s", Weapon[usIndex].ubImpact.toString().padStart(4));
  DrawTextToScreen(sTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT(), BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK, false, RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;
  return usPosY;
}
...
function DisplayRange(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  sTemp = swprintf("%s %s", Weapon[usIndex].usRange.toString().padStart(3), pMessageStrings[Enum333.MSG_METER_ABBREVIATION]);
  DrawTextToScreen(sTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT(), BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK, false, RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;
  return usPosY;
}
...
function DisplayMagazine(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  sTemp = swprintf("%s %s", Weapon[usIndex].ubMagSize.toString().padStart(3), pMessageStrings[Enum333.MSG_ROUNDS_ABBREVIATION]);
  DrawTextToScreen(sTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT(), BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK, false, RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;
  return usPosY;
}
...
function DisplayCaliber(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  if (Item[usIndex].usItemClass == IC_AMMO) {
    zTemp = swprintf("%s", BobbyRayAmmoCaliber[Magazine[Item[usIndex].ubClassIndex].ubCalibre]);
    ...
    zTemp = swprintf("%s", BobbyRayAmmoCaliber[Weapon[Item[usIndex].ubClassIndex].ubCalibre]);
...
function DisplayWeight(usPosY: UINT16, usIndex: UINT16, usFontHeight: UINT16): UINT16 {
  ...
  sTemp = swprintf("%s %s", (GetWeightBasedOnMetricOption(Item[usIndex].ubWeight) / 10).toFixed(2).padStart(3), GetWeightUnitString());
  DrawTextToScreen(sTemp, BOBBYR_ITEM_WEIGHT_NUM_X, usPosY, BOBBYR_ITEM_WEIGHT_NUM_WIDTH, BOBBYR_ITEM_DESC_TEXT_FONT(), BOBBYR_ITEM_DESC_TEXT_COLOR, FONT_MCOLOR_BLACK, false, RIGHT_JUSTIFIED);
  usPosY += usFontHeight + 2;
  return usPosY;
}
...
function DisplayItemNameAndInfo(usPosY: UINT16, usIndex: UINT16, usBobbyIndex: UINT16, fUsed: boolean): void {
  ...
  if (ubPurchaseNumber != BOBBY_RAY_NOT_PURCHASED) {
    ...
    if (ubPurchaseNumber != BOBBY_RAY_NOT_PURCHASED) {
      sTemp = swprintf("%s", BobbyRayPurchases[ubPurchaseNumber].ubNumberPurchased.toString().padStart(4));
  ...
  sText = LoadEncryptedDataFromFile(BOBBYRDESCFILE, uiStartLoc, BOBBYR_ITEM_DESC_INFO_SIZE);
...
function BtnBobbyROrderFormCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnBobbyRHomeButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
