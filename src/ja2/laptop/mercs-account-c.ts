...
function BtnMercAuthorizeButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
...
function BtnMercBackButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function DisplayHiredMercs(): void {
  ...
  for (i = 0; i <= 10; i++) {
    ...
    if (IsMercOnTeam(usMercID) || gMercProfiles[usMercID].iMercMercContractLength != 0) {
      ...
      sTemp = swprintf("%d", gMercProfiles[usMercID].iMercMercContractLength);
