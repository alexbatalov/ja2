...
const MIN_CONDITION_FOR_SAM_SITE_TO_WORK = 80;

// FUNCTIONS FOR DERTERMINING GOOD SECTOR EXIT DATA
const CHECK_DIR_X_DELTA = (WORLD_TILE_X * 4);
const CHECK_DIR_Y_DELTA = (WORLD_TILE_Y * 10);

// get index into aray
const CALCULATE_STRATEGIC_INDEX = (x, y) => (x + (y * MAP_WORLD_X));
const GET_X_FROM_STRATEGIC_INDEX = (i) => (i % MAP_WORLD_X);
const GET_Y_FROM_STRATEGIC_INDEX = (i) => (i / MAP_WORLD_X);

// macros to convert between the 2 different sector numbering systems
const SECTOR_INFO_TO_STRATEGIC_INDEX = (i) => (CALCULATE_STRATEGIC_INDEX(SECTORX(i), SECTORY(i)));
