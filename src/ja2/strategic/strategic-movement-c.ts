...
export function AddPlayerToGroup(ubGroupID: UINT8, pSoldier: SOLDIERTYPE): boolean {
  let pGroup: GROUP;
  ...
  if (!pGroup.pPlayerList) {
    ...
    while (curr.next) {
      if (curr.ubProfileID == pSoldier.ubProfile)
        AssertMsg(0, FormatString("Attempting to add an already existing merc to group (ubProfile=%d).", pSoldier.ubProfile));
      curr = curr.next;
    }
    curr.next = pPlayer;
...
function RemovePlayerFromPGroup(pGroup: GROUP, pSoldier: SOLDIERTYPE): boolean {
...
export function RemovePlayerFromGroup(ubGroupID: UINT8, pSoldier: SOLDIERTYPE): boolean {
  let pGroup: GROUP;
  pGroup = GetGroup(ubGroupID);
  ...
  AssertMsg(pGroup, FormatString("Attempting to RemovePlayerFromGroup( %d, %d ) from non-existant group", ubGroupID, pSoldier.ubProfile));

  return RemovePlayerFromPGroup(pGroup, pSoldier);
}

export function GroupReversingDirectionsBetweenSectors(pGroup: GROUP, ubSectorX: UINT8, ubSectorY: UINT8, fBuildingWaypoints: boolean): boolean {
  ...
  if (pGroup.uiArrivalTime > GetWorldTotalMin()) {
    ...
    AddStrategicEvent(Enum132.EVENT_GROUP_ARRIVAL, pGroup.uiArrivalTime, pGroup.ubGroupID);

    if (pGroup.fPlayer) {
      if ((pGroup.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY) > GetWorldTotalMin()) {
        // Post the about to arrive event
        AddStrategicEvent(Enum132.EVENT_GROUP_ABOUT_TO_ARRIVE, pGroup.uiArrivalTime - ABOUT_TO_ARRIVE_DELAY, pGroup.ubGroupID);
      }
    }
  } else {
...
export function GroupBetweenSectorsAndSectorXYIsInDifferentDirection(pGroup: GROUP, ubSectorX: UINT8, ubSectorY: UINT8): boolean {
  let currDX: INT32;
  let currDY: INT32;
  let newDX: INT32;
  let newDY: INT32;
  ...
  if (ubNumUnalignedAxes > 1) {
    AssertMsg(false, FormatString("Checking a diagonal move for direction change, groupID %d. AM-0", pGroup.ubGroupID));
    return false;
  }
...
export function AddWaypointToPGroup(pGroup: GROUP, ubSectorX: UINT8, ubSectorY: UINT8): boolean // Same, but overloaded
{
  ...
  if (!fReversingDirection) {
    if (ubNumAlignedAxes == 0) {
      AssertMsg(false, FormatString("Invalid DIAGONAL waypoint being added for groupID %d. AM-0", pGroup.ubGroupID));
      return false;
    }

    if (ubNumAlignedAxes >= 2) {
      AssertMsg(false, FormatString("Invalid IDENTICAL waypoint being added for groupID %d. AM-0", pGroup.ubGroupID));
      return false;
    }
...
// NOTE: This does NOT expect a strategic sector ID
export function AddWaypointIDToPGroup(pGroup: GROUP, ubSectorID: UINT8): boolean {
  let ubSectorX: UINT8;
  let ubSectorY: UINT8;
...
export function AddWaypointStrategicIDToPGroup(pGroup: GROUP, uiSectorID: UINT32): boolean {
  let ubSectorX: UINT8;
  let ubSectorY: UINT8;
...
//............................................................
export function CreateNewEnemyGroupDepartingFromSector(uiSector: UINT32, ubNumAdmins: UINT8, ubNumTroops: UINT8, ubNumElites: UINT8): GROUP {
  let pNew: GROUP;
...
function HandleImportantPBIQuote(pSoldier: SOLDIERTYPE, pInitiatingBattleGroup: GROUP): void {
...
function PrepareForPreBattleInterface(pPlayerDialogGroup: GROUP, pInitiatingBattleGroup: GROUP): void {
  ...
  AssertMsg(pPlayer, FormatString("Player group %d doesn't have *any* players in it!  (Finding dialog group)", pPlayerDialogGroup.ubGroupID));

  while (pPlayer != null) {
    pSoldier = pPlayer.pSoldier;

    if (pSoldier.bLife >= OKLIFE && !(pSoldier.uiStatusFlags & SOLDIER_VEHICLE) && !AM_A_ROBOT(pSoldier) && !AM_AN_EPC(pSoldier)) {
  ...
  if (gfTacticalTraversal && pInitiatingBattleGroup == gpTacticalTraversalGroup || pInitiatingBattleGroup && !pInitiatingBattleGroup.fPlayer && pInitiatingBattleGroup.ubSectorX == gWorldSectorX && pInitiatingBattleGroup.ubSectorY == gWorldSectorY && !gbWorldSectorZ) {
    // At least say quote....
    if (ubNumMercs > 0) {
    ...
    if (curr.fPlayer && curr.ubGroupSize) {
      if (!curr.fBetweenSectors) {
        if (curr.ubSectorX == pGroup.ubSectorX && curr.ubSectorY == pGroup.ubSectorY && !curr.ubSectorZ) {
          if (!GroupHasInTransitDeadOrPOWMercs(curr) && (!IsGroupTheHelicopterGroup(curr) || !fHelicopterIsAirBorne) && (!curr.fVehicle || NumberMercsInVehicleGroup(curr))) {
...
// this, and continue on.
    ...
    if (!fCombatAbleMerc) {
      ...
      if (fMilitiaPresent) {
        ...
        pSectorStr = GetSectorIDString(pGroup.ubSectorX, pGroup.ubSectorY, pGroup.ubSectorZ, true);
...
export function AttemptToMergeSeparatedGroups(pGroup: GROUP, fDecrementTraversals: boolean): boolean {
...
function AwardExperienceForTravelling(pGroup: GROUP): void {
  ...
  while (pPlayerGroup) {
    ...
    if (pSoldier && !AM_A_ROBOT(pSoldier) && !AM_AN_EPC(pSoldier) && !(pSoldier.uiStatusFlags & SOLDIER_VEHICLE)) {
...
export function GroupArrivedAtSector(ubGroupID: UINT8, fCheckForBattle: boolean, fNeverLeft: boolean): void {
  ...
  if (fExceptionQueue || fCheckForBattle && gTacticalStatus.fEnemyInSector && FindMovementGroupInSector(gWorldSectorX, gWorldSectorY, true) && (pGroup.ubNextX != gWorldSectorX || pGroup.ubNextY != gWorldSectorY || gbWorldSectorZ > 0) || AreInMeanwhile() ||
    ...
    if (!AddStrategicEvent(Enum132.EVENT_GROUP_ARRIVAL, pGroup.uiArrivalTime, pGroup.ubGroupID))
  ...
  if (pGroup.fPlayer) {
    ...
    if (pGroup.ubSectorZ == 0) {
      ...
      let bTownId: INT8 = GetTownIdForSector(pGroup.ubSectorX, pGroup.ubSectorY);
    ...
    if (pGroup.fVehicle == false) {
      ...
      while (curr) {
        ...
        if (curr.pSoldier.ubStrategicInsertionCode != Enum175.INSERTION_CODE_PRIMARY_EDGEINDEX && curr.pSoldier.ubStrategicInsertionCode != Enum175.INSERTION_CODE_SECONDARY_EDGEINDEX) {
      ...
      if (pGroup.pPlayerList) {
        ...
        if (GroupAtFinalDestination(pGroup) && (pGroup.ubSectorZ == 0) && !fNeverLeft) {
          ...
          if (pGroup.pPlayerList.pSoldier.bAssignment < Enum117.ON_DUTY) {
            // squad
            ScreenMsg(FONT_MCOLOR_DKRED, MSG_INTERFACE, pMessageStrings[Enum333.MSG_ARRIVE], pAssignmentStrings[pGroup.pPlayerList.pSoldier.bAssignment], pMapVertIndex[pGroup.pPlayerList.pSoldier.sSectorY], pMapHortIndex[pGroup.pPlayerList.pSoldier.sSectorX]);
          } else {
            // a loner
            ScreenMsg(FONT_MCOLOR_DKRED, MSG_INTERFACE, pMessageStrings[Enum333.MSG_ARRIVE], pGroup.pPlayerList.pSoldier.name, pMapVertIndex[pGroup.pPlayerList.pSoldier.sSectorY], pMapHortIndex[pGroup.pPlayerList.pSoldier.sSectorX]);
          }
        }
      }
    } else // vehicle player group
    {
      ...
      if (!fGroupDestroyed) {
        ...
        if (GroupAtFinalDestination(pGroup) && (pGroup.ubSectorZ == 0) && !fNeverLeft) {
          ScreenMsg(FONT_MCOLOR_DKRED, MSG_INTERFACE, pMessageStrings[Enum333.MSG_ARRIVE], pVehicleStrings[pVehicleList[iVehId].ubVehicleType], pMapVertIndex[pGroup.ubSectorY], pMapHortIndex[pGroup.ubSectorX]);
        }
      }
    }
    ...
    if (!fGroupDestroyed) {
      ...
      if (!pGroup.fVehicle || !IsGroupTheHelicopterGroup(pGroup)) {
        ...
        if (SECTOR(pGroup.ubSectorX, pGroup.ubSectorY) == Enum123.SEC_I16 && fFirstTimeInSector) {
...
function HandleNonCombatGroupArrival(pGroup: GROUP, fMainGroup: boolean, fNeverLeft: boolean): void {
...
function HandleOtherGroupsArrivingSimultaneously(ubSectorX: UINT8, ubSectorY: UINT8, ubSectorZ: UINT8): void {
  ...
  while (pEvent && pEvent.uiTimeStamp <= uiCurrTimeStamp) {
    if (pEvent.ubCallbackID == Enum132.EVENT_GROUP_ARRIVAL && !(pEvent.ubFlags & SEF_DELETION_PENDING)) {
...
function PrepareGroupsForSimultaneousArrival(): void {
  ...
  while (pGroup) {
    ...
    if (pGroup != gpPendingSimultaneousGroup && pGroup.fPlayer && pGroup.fBetweenSectors && pGroup.ubNextX == gpPendingSimultaneousGroup.ubSectorX && pGroup.ubNextY == gpPendingSimultaneousGroup.ubSectorY && !IsGroupTheHelicopterGroup(pGroup)) {
...
function PossibleToCoordinateSimultaneousGroupArrivals(pFirstGroup: GROUP): boolean {
  let pGroup: GROUP | null;
  let ubNumNearbyGroups: UINT8 = 0;

  // If the user has already been asked, then don't ask the question again!
  if (pFirstGroup.uiFlags & (GROUPFLAG_SIMULTANEOUSARRIVAL_APPROVED | GROUPFLAG_SIMULTANEOUSARRIVAL_CHECKED) || IsGroupTheHelicopterGroup(pFirstGroup)) {
    return false;
  }
  ...
  pGroup = gpGroupList;
  while (pGroup) {
    if (pGroup != pFirstGroup && pGroup.fPlayer && pGroup.fBetweenSectors && pGroup.ubNextX == pFirstGroup.ubSectorX && pGroup.ubNextY == pFirstGroup.ubSectorY && !(pGroup.uiFlags & GROUPFLAG_SIMULTANEOUSARRIVAL_CHECKED) && !IsGroupTheHelicopterGroup(pGroup)) {
  ...
  if (ubNumNearbyGroups) {
    ...
    str = swprintf(pStr, pEnemyType, String.fromCharCode('A'.charCodeAt(0) + gpPendingSimultaneousGroup.ubSectorY - 1), gpPendingSimultaneousGroup.ubSectorX); // Sector location
    str += "  ";
  ...
  while (pGroup) {
    if (!pGroup.fPlayer) {
      ...
      if (pGroup.uiArrivalTime < pPlayerGroup.uiArrivalTime) {
        ...
        if (pGroup.ubNextX == pPlayerGroup.ubSectorX && pGroup.ubNextY == pPlayerGroup.ubSectorY && pGroup.ubSectorX == pPlayerGroup.ubNextX && pGroup.ubSectorY == pPlayerGroup.ubNextY) {
...
function InitiateGroupMovementToNextSector(pGroup: GROUP): void {
  ...
  if (dx && dy) {
    ...
    AssertMsg(0, FormatString("Attempting to move to waypoint in a diagonal direction from sector %d,%d to sector %d,%d", pGroup.ubSectorX, pGroup.ubSectorY, wp.x, wp.y));
  }
  if (!dx && !dy) // Can't move to position currently at!
    AssertMsg(0, FormatString("Attempting to move to waypoint %d, %d that you are already at!", wp.x, wp.y));
...
export function FindTravelTimeBetweenWaypoints(pSource: WAYPOINT, pDest: WAYPOINT, pGroup: GROUP): INT32 {
  let ubStart: UINT8 = 0;
  let ubEnd: UINT8 = 0;
  let iDelta: INT32 = 0;
  ...
  for (ubCurrentSector = ubStart; ubCurrentSector != ubEnd; ubCurrentSector += iDelta) {
    ...
    if (iThisCostInTime == 0xffffffff) {
      AssertMsg(0, FormatString("Group %d (%s) attempting illegal move from sector %d, dir %d (%s).", pGroup.ubGroupID, (pGroup.fPlayer) ? "Player" : "AI", ubCurrentSector, ubDirection, gszTerrain[SectorInfo[ubCurrentSector].ubTraversability[ubDirection]]));
    }
...
export function GetSectorMvtTimeForGroup(ubSector: UINT8, ubDirection: UINT8, pGroup: GROUP): INT32 {
  ...
  while (curr) {
    ...
    if (curr.fPlayer == true) {
      ...
      if (!fHelicopterGroup || !fShowAircraftFlag) {
        ...
        if ((gfDisplayPotentialRetreatPaths == true) && (sBattleSector == sSource) || (gfDisplayPotentialRetreatPaths == false) && (curr.fBetweenSectors == true)) {
...
export function MoveAllGroupsInCurrentSectorToSector(ubSectorX: UINT8, ubSectorY: UINT8, ubSectorZ: UINT8): void {
  ...
  pGroup = gpGroupList;
  while (pGroup) {
    if (pGroup.fPlayer && pGroup.ubSectorX == gWorldSectorX && pGroup.ubSectorY == gWorldSectorY && pGroup.ubSectorZ == gbWorldSectorZ && !pGroup.fBetweenSectors) {
...
function CheckMembersOfMvtGroupAndComplainAboutBleeding(pSoldier: SOLDIERTYPE): void {
...
export function CalculateGroupRetreatSector(pGroup: GROUP): void {
  ...
  if (pSector.ubTraversability[Enum186.NORTH_STRATEGIC_MOVE] != Enum127.GROUNDBARRIER && pSector.ubTraversability[Enum186.NORTH_STRATEGIC_MOVE] != Enum127.EDGEOFWORLD) {
    pGroup.ubPrevX = pGroup.ubSectorX;
    pGroup.ubPrevY = pGroup.ubSectorY - 1;
  } else if (pSector.ubTraversability[Enum186.EAST_STRATEGIC_MOVE] != Enum127.GROUNDBARRIER && pSector.ubTraversability[Enum186.EAST_STRATEGIC_MOVE] != Enum127.EDGEOFWORLD) {
    pGroup.ubPrevX = pGroup.ubSectorX + 1;
    pGroup.ubPrevY = pGroup.ubSectorY;
  } else if (pSector.ubTraversability[Enum186.WEST_STRATEGIC_MOVE] != Enum127.GROUNDBARRIER && pSector.ubTraversability[Enum186.WEST_STRATEGIC_MOVE] != Enum127.EDGEOFWORLD) {
    pGroup.ubPrevX = pGroup.ubSectorX - 1;
    pGroup.ubPrevY = pGroup.ubSectorY;
  } else if (pSector.ubTraversability[Enum186.SOUTH_STRATEGIC_MOVE] != Enum127.GROUNDBARRIER && pSector.ubTraversability[Enum186.SOUTH_STRATEGIC_MOVE] != Enum127.EDGEOFWORLD) {
    pGroup.ubPrevX = pGroup.ubSectorX;
    pGroup.ubPrevY = pGroup.ubSectorY + 1;
  } else {
...
export function FindMovementGroupInSector(ubSectorX: UINT8, ubSectorY: UINT8, fPlayer: boolean): GROUP | null {
  let pGroup: GROUP | null;
  pGroup = gpGroupList;
  while (pGroup) {
    if (pGroup.fPlayer) {
      // NOTE: These checks must always match the INVOLVED group checks in PBI!!!
      if (fPlayer && pGroup.ubGroupSize && !pGroup.fBetweenSectors && pGroup.ubSectorX == ubSectorX && pGroup.ubSectorY == ubSectorY && !pGroup.ubSectorZ && !GroupHasInTransitDeadOrPOWMercs(pGroup) && (!IsGroupTheHelicopterGroup(pGroup) || !fHelicopterIsAirBorne)) {
        return pGroup;
      }
...
export function GroupAtFinalDestination(pGroup: GROUP): boolean {
  ...
  if (pGroup.ubMoveType != Enum185.ONE_WAY)
...
export function GroupWillMoveThroughSector(pGroup: GROUP, ubSectorX: UINT8, ubSectorY: UINT8): boolean {
  let wp: WAYPOINT | null;
  let i: INT32;
  let dx: INT32;
  let dy: INT32;
...
export function AddFuelToVehicle(pSoldier: SOLDIERTYPE, pVehicle: SOLDIERTYPE): void {
...
function ReportVehicleOutOfGas(iVehicleID: INT32, ubSectorX: UINT8, ubSectorY: UINT8): void {
  ...
  str = swprintf(gzLateLocalizedString[5], pVehicleStrings[pVehicleList[iVehicleID].ubVehicleType], String.fromCharCode(ubSectorY + 'A'.charCodeAt(0) - 1), ubSectorX);
  DoScreenIndependantMessageBox(str, MSG_BOX_FLAG_OK, null);
}

function SetLocationOfAllPlayerSoldiersInGroup(pGroup: GROUP, sSectorX: INT16, sSectorY: INT16, bSectorZ: INT8): void {
  let pPlayer: PLAYERGROUP | null = null;
  let pSoldier: SOLDIERTYPE;
...
function TestForBloodcatAmbush(pGroup: GROUP): boolean {
  ...
  if (ubSectorID == Enum123.SEC_N5 || ubSectorID == Enum123.SEC_I16) {
    ...
    if (gfAutoAmbush || PreChance(ubChance)) {
      ...
      pSector.bBloodCats = Math.min(pSector.bBloodCats, pSector.bBloodCatPlacements);
    }
  ...
  if (!fAlreadyAmbushed && ubSectorID != Enum123.SEC_N5 && pSector.bBloodCats > 0 && !pGroup.fVehicle && !NumEnemiesInSector(pGroup.ubSectorX, pGroup.ubSectorY)) {
...
export function SetGroupArrivalTime(pGroup: GROUP, uiArrivalTime: UINT32): void {
  ...
  if (IsGroupTheHelicopterGroup(pGroup)) {
    ...
    if (uiArrivalTime > (GetWorldTotalMin() + pGroup.uiTraverseTime)) {
      AssertMsg(false, FormatString("SetGroupArrivalTime: Setting invalid arrival time %d for group %d, WorldTime = %d, TraverseTime = %d", uiArrivalTime, pGroup.ubGroupID, GetWorldTotalMin(), pGroup.uiTraverseTime));
...
// look for NPCs to stop for, anyone is too tired to keep going, if all OK rebuild waypoints & continue movement
export function PlayerGroupArrivedSafelyInSector(pGroup: GROUP, fCheckForNPCs: boolean): void {
...
function HandlePlayerGroupEnteringSectorToCheckForNPCsOfNote(pGroup: GROUP): boolean {
  ...
  sString = swprintf(pLandMarkInSectorString[0], pGroup.pPlayerList.pSoldier.bAssignment + 1, wSectorName);
...
function WildernessSectorWithAllProfiledNPCsNotSpokenWith(sSectorX: INT16, sSectorY: INT16, bSectorZ: INT8): boolean {
  ...
  for (ubProfile = FIRST_RPC; ubProfile < NUM_PROFILES; ubProfile++) {
    ...
    if (pProfile.sSectorX == sSectorX && pProfile.sSectorY == sSectorY && pProfile.bSectorZ == bSectorZ) {
      ...
      if ((pProfile.ubLastDateSpokenTo == 0) && !(pProfile.ubMiscFlags & (PROFILE_MISC_FLAG_RECRUITED | PROFILE_MISC_FLAG_EPCACTIVE))) {
...
export function DoesPlayerExistInPGroup(ubGroupID: UINT8, pSoldier: SOLDIERTYPE): boolean {
  let pGroup: GROUP;
  let curr: PLAYERGROUP | null;
...
export function GroupHasInTransitDeadOrPOWMercs(pGroup: GROUP): boolean {
  ...
  while (pPlayer) {
    if (pPlayer.pSoldier) {
      if ((pPlayer.pSoldier.bAssignment == Enum117.IN_TRANSIT) || (pPlayer.pSoldier.bAssignment == Enum117.ASSIGNMENT_POW) || (pPlayer.pSoldier.bAssignment == Enum117.ASSIGNMENT_DEAD)) {
        // yup!
        return true;
      }
    }
