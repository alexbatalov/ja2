...
export function StrategicHandlePlayerTeamMercDeath(pSoldier: SOLDIERTYPE): void {
  ...
  if (pSoldier.ubProfile != NO_PROFILE) {
    ...
    if (pKiller && pKiller.bTeam == OUR_TEAM) {
      AddHistoryToPlayersLog(Enum83.HISTORY_MERC_KILLED_CHARACTER, pSoldier.ubProfile, GetWorldTotalMin(), sSectorX, sSectorY);
    } else {
      AddHistoryToPlayersLog(Enum83.HISTORY_MERC_KILLED, pSoldier.ubProfile, GetWorldTotalMin(), sSectorX, sSectorY);
    }
  }
  ...
  if (!AM_AN_EPC(pSoldier) && !AM_A_ROBOT(pSoldier)) {
    ...
    HandleMoraleEvent(pSoldier, Enum234.MORALE_TEAMMATE_DIED, pSoldier.sSectorX, pSoldier.sSectorY, pSoldier.bSectorZ);
  }
...
export function MercDailyUpdate(): void {
  ...
  // debug message
  ScreenMsg(MSG_FONT_RED, MSG_DEBUG, "%s - Doing MercDailyUpdate", gswzWorldTimeStr);
  ...
  for (pSoldier = MercPtrs[cnt]; cnt <= bLastTeamID; cnt++, pSoldier = MercPtrs[cnt]) {
    ...
    if ((pSoldier.bActive) && (pSoldier.bAssignment != Enum117.ASSIGNMENT_POW) && (pSoldier.bAssignment != Enum117.IN_TRANSIT)) {
      ...
      if (SoldierHasWorseEquipmentThanUsedTo(pSoldier)) {
        ...
        AddSameDayStrategicEvent(Enum132.EVENT_MERC_COMPLAIN_EQUIPMENT, 360 + Random(1080), pSoldier.ubProfile);
      }
      ...
      if (pSoldier.ubProfile >= FIRST_RPC && pSoldier.ubProfile < FIRST_NPC) {
        ...
        if (iMoneyOwedToMerc != 0) {
          ...
          if (LaptopSaveInfo.iCurrentBalance >= iMoneyOwedToMerc) {
            ...
            AddTransactionToPlayersBook(Enum80.PAYMENT_TO_NPC, pSoldier.ubProfile, GetWorldTotalMin(), -iMoneyOwedToMerc);
            ...
            ScreenMsg(FONT_MCOLOR_WHITE, MSG_INTERFACE, pMessageStrings[Enum333.MSG_CANT_AFFORD_TO_PAY_NPC_DAILY_SALARY_MSG], gMercProfiles[pSoldier.ubProfile].zNickname, zMoney);
            ...
            if ((gMercProfiles[pSoldier.ubProfile].iBalance - sSalary) <= -(sSalary * NUM_DAYS_TILL_UNPAID_RPC_QUITS)) {
              ...
              AddSameDayStrategicEvent(Enum132.EVENT_RPC_WHINE_ABOUT_PAY, MERC_ARRIVE_TIME_SLOT_1, pSoldier.ubID);
            }
          }
        }
      }
      ...
      if ((pSoldier.bActive) && (pSoldier.bAssignment == Enum117.ASSIGNMENT_POW)) {
        pSoldier.iEndofContractTime += 1440;
      }
    }

    // if active, here, & alive (POW is ok, don't care)
    if ((pSoldier.bActive) && (pSoldier.bAssignment != Enum117.ASSIGNMENT_DEAD) && (pSoldier.bAssignment != Enum117.IN_TRANSIT)) {
...
export function SoldierHasWorseEquipmentThanUsedTo(pSoldier: SOLDIERTYPE): boolean {
  let cnt: INT32;
  let usItem: UINT16;
...
export function MercComplainAboutEquipment(ubProfile: UINT8): void {
  ...
  if (pSoldier != null) {
    if (pSoldier.bLife >= OKLIFE && pSoldier.fMercAsleep != true && pSoldier.bAssignment < Enum117.ON_DUTY) {
  ...
  for (pSoldier = MercPtrs[bMercID]; bMercID <= bLastTeamID; bMercID++, pSoldier = MercPtrs[bMercID]) {
    ...
    if (pSoldier.bActive && pSoldier.bAssignment < Enum117.ON_DUTY) {
      ...
      for (pOtherSoldier = MercPtrs[bOtherID]; bOtherID <= bLastTeamID; bOtherID++, pOtherSoldier = MercPtrs[bOtherID]) {
        ...
        if (bOtherID != bMercID && pOtherSoldier.bActive && pOtherSoldier.bAssignment < Enum117.ON_DUTY) {
          if (fSameGroupOnly) {
                ...
                if (pProfile.bLearnToHate == ubOtherProfileID) {
                  if (pProfile.bLearnToHateCount > 0) {
                    ...
                    if (pProfile.bLearnToHateCount == 0 && pSoldier.bInSector && gTacticalStatus.fEnemyInSector) {
                      ...
                      if (pSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__MERC || (pSoldier.ubWhatKindOfMercAmI == Enum260.MERC_TYPE__NPC && (pSoldier.ubProfile == Enum268.DEVIN || pSoldier.ubProfile == Enum268.SLAY || pSoldier.ubProfile == Enum268.IGGY || pSoldier.ubProfile == Enum268.CONRAD))) {
                        // Leave now! ( handle equipment too )....
    ...
    if (pSoldier.bActive) {
      ...
      if ((pSoldier.bCamo > 0) && (!(HAS_SKILL_TRAIT(pSoldier, Enum269.CAMOUFLAGED)))) {
        ...
        if (pSoldier.bCamo <= 0) {
          ...
          ScreenMsg(FONT_MCOLOR_LTYELLOW, MSG_INTERFACE, Message[Enum334.STR_CAMMO_WORN_OFF], pSoldier.name);
