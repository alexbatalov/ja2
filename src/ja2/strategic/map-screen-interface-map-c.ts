...
function ShowOnDutyTeam(sMapX: INT16, sMapY: INT16): INT32 {
  ...
  while (gCharactersList[ubCounter].fValid) {
    ...
    if (!(pSoldier->uiStatusFlags & SOLDIER_VEHICLE) && (pSoldier->sSectorX == sMapX) && (pSoldier->sSectorY == sMapY) && (pSoldier->bSectorZ == iCurrentMapSectorZ) && ((pSoldier->bAssignment < ON_DUTY) || ((pSoldier->bAssignment == VEHICLE) && (pSoldier->iVehicleId != iHelicopterVehicleId))) && (pSoldier->bLife > 0) && (!PlayerIDGroupInMotion(pSoldier->ubGroupID))) {
...
function ShowAssignedTeam(sMapX: INT16, sMapY: INT16, iCount: INT32): INT32 {
  ...
  while (gCharactersList[ubCounter].fValid) {
    ...
    if (!(pSoldier->uiStatusFlags & SOLDIER_VEHICLE) && (pSoldier->sSectorX == sMapX) && (pSoldier->sSectorY == sMapY) && (pSoldier->bSectorZ == iCurrentMapSectorZ) && (pSoldier->bAssignment >= ON_DUTY) && (pSoldier->bAssignment != VEHICLE) && (pSoldier->bAssignment != IN_TRANSIT) && (pSoldier->bAssignment != ASSIGNMENT_POW) && (pSoldier->bLife > 0) && (!PlayerIDGroupInMotion(pSoldier->ubGroupID))) {
  ...
  if (iHelicopterVehicleId != -1) {
    ...
    if (pVehicleList[iHelicopterVehicleId].ubMovementGroup != 0) {
      ...
      */
      ...
      AssertMsg((x >= 0) && (x < 640), String("DisplayPositionOfHelicopter: Invalid x = %d.  At %d,%d.  Next %d,%d.  Min/Max X = %d/%d", x, pGroup->ubSectorX, pGroup->ubSectorY, pGroup->ubNextX, pGroup->ubNextY, minX, maxX));

      AssertMsg((y >= 0) && (y < 480), String("DisplayPositionOfHelicopter: Invalid y = %d.  At %d,%d.  Next %d,%d.  Min/Max Y = %d/%d", y, pGroup->ubSectorX, pGroup->ubSectorY, pGroup->ubNextX, pGroup->ubNextY, minY, maxY));
...
function CreateDestroyMilitiaSectorButtons(): void {
  ...
  if (sOldSectorValue == sSectorMilitiaMapSector && fShowMilitia && sSelectedMilitiaTown && !fCreated && sSectorMilitiaMapSector != -1) {
    ...
    for (iCounter = 0; iCounter < 3; iCounter++) {
      ...
      giMapMilitiaButton[iCounter] = QuickCreateButton(giMapMilitiaButtonImage[iCounter], sX, sY, BUTTON_TOGGLE, MSYS_PRIORITY_HIGHEST - 1, BtnGenericMouseMoveButtonCallback, (GUI_CALLBACK)MilitiaButtonCallback);
...
function CreateMilitiaPanelBottomButton(): void {
  ...
  giMapMilitiaButton[3] = QuickCreateButton(giMapMilitiaButtonImage[3], MAP_MILITIA_BOX_POS_X + MAP_MILITIA_BOX_AUTO_BOX_X, MAP_MILITIA_BOX_POS_Y + MAP_MILITIA_BOX_AUTO_BOX_Y, BUTTON_TOGGLE, MSYS_PRIORITY_HIGHEST - 1, BtnGenericMouseMoveButtonCallback, (GUI_CALLBACK)MilitiaAutoButtonCallback);

  giMapMilitiaButton[4] = QuickCreateButton(giMapMilitiaButtonImage[4], MAP_MILITIA_BOX_POS_X + MAP_MILITIA_BOX_DONE_BOX_X, MAP_MILITIA_BOX_POS_Y + MAP_MILITIA_BOX_AUTO_BOX_Y, BUTTON_TOGGLE, MSYS_PRIORITY_HIGHEST - 1, BtnGenericMouseMoveButtonCallback, (GUI_CALLBACK)MilitiaDoneButtonCallback);
...
function RenderShadingForUnControlledSectors(): void {
  ...
  for (iCounter = 0; iCounter < 9; iCounter++) {
    ...
    if ((StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(sCurrentSectorValue)].bNameId != BLANK_SECTOR) && ((StrategicMap[SECTOR_INFO_TO_STRATEGIC_INDEX(sCurrentSectorValue)].fEnemyControlled) || (NumHostilesInSector((INT16)SECTORX(sCurrentSectorValue), (INT16)SECTORY(sCurrentSectorValue), 0)))) {
...
function CanMercsScoutThisSector(sSectorX: INT16, sSectorY: INT16, bSectorZ: INT8): BOOLEAN {
  ...
  for (iCounter = iFirstId; iCounter <= iLastId; iCounter++) {
    ...
    if ((pSoldier->bAssignment == IN_TRANSIT) || (pSoldier->bAssignment == ASSIGNMENT_POW) || (pSoldier->bAssignment == ASSIGNMENT_DEAD) || (pSoldier->fMercAsleep == TRUE) || (pSoldier->bLife < OKLIFE)) {
      continue;
    }
