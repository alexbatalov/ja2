...
let gubCurrentNpcInSector: UINT8[] /* [QUEST_DBS_SIZE_NPC_ARRAY] */ = createArray(QUEST_DBS_SIZE_NPC_ARRAY, 0);
...
function EnterQuestDebugSystem(): boolean {
  ...
  MSYS_DefineRegion(gQuestDebugSysScreenRegions, 0, 0, 640, 480, MSYS_PRIORITY_HIGH, Enum317.CURSOR_LAPTOP_SCREEN, MSYS_NO_CALLBACK, MSYS_NO_CALLBACK);
  // Add region
  ...
  // NPC log button
  zName = swprintf("%s - (%s)", QuestDebugText[Enum167.QUEST_DBS_NPC_LOG_BUTTON], gfNpcLogButton ? "On" : "Off");
  ...
  for (i = 0; i < QUEST_DBS_NUM_DISPLAYED_QUESTS; i++) {
    MSYS_DefineRegion(gQuestListRegion[i], usPosX, usPosY, (usPosX + QUEST_DBS_FIRST_SECTION_WIDTH), (usPosY + usFontHeight), MSYS_PRIORITY_HIGH + 2, Enum317.CURSOR_WWW, MSYS_NO_CALLBACK, ScrollQuestListRegionCallBack); // CURSOR_LAPTOP_SCREEN
    // Add region
  ...
  for (i = 0; i < QUEST_DBS_NUM_DISPLAYED_FACTS; i++) {
    MSYS_DefineRegion(gFactListRegion[i], usPosX, usPosY, (usPosX + QUEST_DBS_SECOND_SECTION_WIDTH), (usPosY + usFontHeight), MSYS_PRIORITY_HIGH + 2, Enum317.CURSOR_WWW, MSYS_NO_CALLBACK, ScrollFactListRegionCallBack); // CURSOR_LAPTOP_SCREEN
    // Add region
  ...
  if (giHaveSelectedNPC != -1) {
    ...
    if (gfUseLocalNPCs)
      zItemDesc = swprintf("%d - %s", gubCurrentNpcInSector[giHaveSelectedNPC], gMercProfiles[gubCurrentNpcInSector[giHaveSelectedNPC]].zNickname);
    else
      zItemDesc = swprintf("%d - %s", giHaveSelectedNPC, gMercProfiles[giHaveSelectedNPC].zNickname);
...
function HandleQuestDebugSystem(): void {
  ...
  if (gpActiveListBox.ubCurScrollBoxAction != Enum166.QD_DROP_DOWN_NO_ACTION) {
    CreateDestroyDisplaySelectNpcDropDownBox();
...
function DisplayCurrentGridNo(): void {
  if (gsQdsEnteringGridNo != 0) {
    ...
    zTemp = swprintf("%s:  %d", QuestDebugText[Enum167.QUEST_DBS_CURRENT_GRIDNO], gsQdsEnteringGridNo);
    ...
    if (!HandleTextInput(Event) && Event.usEvent == KEY_DOWN) {
      switch (Event.usParam) {
        ...
        case 'd'.charCodeAt(0): {
          ...
          zTemp = swprintf("Items dropped will be in %s condition", gfDropDamagedItems ? "DAMAGED" : "PERFECT");
...
function BtnQuestDebugExitButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
...
function BtnQuestDebugCurNPCButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugCurItemButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
/* static */ let CreateDestroyDisplaySelectNpcDropDownBox__fMouseRegionsCreated: boolean = false;
  ...
  switch (gpActiveListBox.ubCurScrollBoxAction) {
    ...
    case Enum166.QD_DROP_DOWN_CREATE: {
      ...
      for (i = 0; i < gpActiveListBox.usNumDisplayedItems; i++) {
        MSYS_DefineRegion(gSelectedNpcListRegion[i], usPosX, (usPosY), (usPosX + gpActiveListBox.usScrollWidth), (usPosY + usFontHeight), MSYS_PRIORITY_HIGH + 20, Enum317.CURSOR_WWW, SelectNpcListMovementCallBack, SelectNpcListRegionCallBack);
      ...
      for (i = 0; i < QUEST_DBS_NUM_INCREMENTS_IN_SCROLL_BAR; i++) {
        MSYS_DefineRegion(gScrollAreaRegion[i], usPosX, usPosY, (usPosX + gpActiveListBox.usScrollBarWidth), (usPosY + gpActiveListBox.usScrollBarHeight), MSYS_PRIORITY_HIGH + 20, Enum317.CURSOR_WWW, ScrollAreaMovementCallBack, ScrollAreaRegionCallBack);
      ...
      MSYS_DefineRegion(gScrollArrowsRegion[0], usPosX, (usPosY), (usPosX + gpActiveListBox.usScrollBarWidth), (usPosY + gpActiveListBox.usScrollArrowHeight), MSYS_PRIORITY_HIGH + 20, Enum317.CURSOR_WWW, MSYS_NO_CALLBACK, ScrollArrowsRegionCallBack);
      ...
      MSYS_DefineRegion(gScrollArrowsRegion[1], usPosX, usPosY, (usPosX + gpActiveListBox.usScrollBarWidth), (usPosY + gpActiveListBox.usScrollArrowHeight), MSYS_PRIORITY_HIGH + 20, Enum317.CURSOR_WWW, MSYS_NO_CALLBACK, ScrollArrowsRegionCallBack);
      ...
      if (!gfBackgroundMaskEnabled) {
        MSYS_DefineRegion(gQuestTextEntryDebugDisableScreenRegion, 0, 0, 640, 480, MSYS_PRIORITY_HIGH + 15, Enum317.CURSOR_LAPTOP_SCREEN, MSYS_NO_CALLBACK, QuestDebugTextEntryDisableScreenRegionCallBack);
    ...
    case Enum166.QD_DROP_DOWN_DESTROY: {
      ...
      if (CreateDestroyDisplaySelectNpcDropDownBox__fMouseRegionsCreated) {
        ...
        CreateDestroyDisplaySelectNpcDropDownBox__fMouseRegionsCreated = false;
...
function DisplaySelectedNPC(): void {
  ...
  for (i = gpActiveListBox.usItemDisplayedOnTopOfList; i < gpActiveListBox.usItemDisplayedOnTopOfList + gpActiveListBox.usNumDisplayedItems; i++) {
    ...
    ({ sX: usLocationX, sY: usLocationY } = FindFontRightCoordinates(gpActiveListBox.usScrollPosX, usPosY, gpActiveListBox.usScrollWidth, 0, sTempString, QUEST_DBS_FONT_LISTBOX_TEXT()));
  ...
  if (gpActiveListBox.sCurSelectedItem >= 0) {
    ...
    if (gfUseLocalNPCs) {
      ...
      sTempString = GetDebugLocationString(gubCurrentNpcInSector[gpActiveListBox.sCurSelectedItem]);
    ...
    if (gfUseLocalNPCs)
      zButtonName = swprintf("%d - %s", gubCurrentNpcInSector[gpActiveListBox.sCurSelectedItem], gMercProfiles[gubCurrentNpcInSector[gpActiveListBox.sCurSelectedItem]].zNickname);
    else
      zButtonName = swprintf("%d - %s", gpActiveListBox.sCurSelectedItem, gMercProfiles[gpActiveListBox.sCurSelectedItem].zNickname);
...
function DisplaySelectedItem(): void {
  ...
  if (gpActiveListBox.sCurSelectedItem >= 0) {
    ...
    zButtonName = swprintf("%d - %s", gpActiveListBox.sCurSelectedItem, zItemName);
...
function SelectNpcListRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
...
function SelectNpcListMovementCallBack(pRegion: MOUSE_REGION, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
...
function ScrollArrowsRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
...
function ScrollAreaRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
...
function ScrollAreaMovementCallBack(pRegion: MOUSE_REGION, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LOST_MOUSE) {
    pRegion.uiFlags &= (~BUTTON_CLICKED_ON);
...
function BtnQuestDebugAddNpcToLocationButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    zTemp = swprintf("%s where %s will be added.", QuestDebugText[Enum167.QUEST_DBS_ENTER_GRID_NUM], gMercProfiles[gNpcListBox.sCurSelectedItem].zNickname);
...
function BtnQuestDebugAddItemToLocationButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugGiveItemToNPCButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugChangeDayButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    zTemp = swprintf("%s   Current Day is %d", QuestDebugText[Enum167.QUEST_DBS_PLEASE_ENTER_DAY], GetWorldDay());
...
function BtnQuestDebugViewNPCInvButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugRestoreNPCInvButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugNPCLogButtonButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
...
function BtnQuestDebugNPCRefreshButtonButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    if (fRetVal) {
      zTemp = swprintf("%s %s", QuestDebugText[Enum167.QUEST_DBS_REFRESH_OK], gMercProfiles[ubMercID].zNickname);
    } else {
      zTemp = swprintf("%s %s", QuestDebugText[Enum167.QUEST_DBS_REFRESH_FAILED], gMercProfiles[ubMercID].zNickname);
    }
...
function BtnQuestDebugStartMercTalkingButtonButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
/* static */ let CreateDestroyDisplayTextEntryBox__fMouseRegionCreated: boolean = false;
...
function CreateDestroyDisplayTextEntryBox(ubAction: UINT8, pString: string | null /* STR16 */, EntryCallBack: TEXT_ENTRY_CALLBACK | null): boolean {
  switch (ubAction) {
    case Enum166.QD_DROP_DOWN_NO_ACTION: {
    } break;

    case Enum166.QD_DROP_DOWN_CREATE: {
      if (CreateDestroyDisplayTextEntryBox__fMouseRegionCreated)
        break;
      ...
      // create a mask to block out the screen
      if (!gfBackgroundMaskEnabled) {
        MSYS_DefineRegion(gQuestTextEntryDebugDisableScreenRegion, 0, 0, 640, 480, MSYS_PRIORITY_HIGH + 40, Enum317.CURSOR_LAPTOP_SCREEN, MSYS_NO_CALLBACK, QuestDebugTextEntryDisableScreenRegionCallBack);
        MSYS_AddRegion(gQuestTextEntryDebugDisableScreenRegion);
        gfBackgroundMaskEnabled = true;
      }
      ...
      CreateDestroyDisplayTextEntryBox__TextEntryCallback = <TEXT_ENTRY_CALLBACK>EntryCallBack;
    } break;
    ...
    case Enum166.QD_DROP_DOWN_DESTROY: {
      ...
      if (ubAction != Enum166.QD_DROP_DOWN_CANCEL)
        CreateDestroyDisplayTextEntryBox__TextEntryCallback(iTextEntryNumber);
    } break;
    ...
    case Enum166.QD_DROP_DOWN_DISPLAY: {
      ...
      DisplayWrappedString(QUEST_DBS_TEB_X + 10, QUEST_DBS_TEB_Y + 10, QUEST_DBS_TEB_WIDTH - 20, 2, QUEST_DBS_FONT_TEXT_ENTRY(), QUEST_DBS_COLOR_TEXT_ENTRY, CreateDestroyDisplayTextEntryBox__zString, FONT_MCOLOR_BLACK, false, CENTER_JUSTIFIED);
...
function QuestDebugTextEntryDisableScreenRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    gpActiveListBox.ubCurScrollBoxAction = Enum166.QD_DROP_DOWN_DESTROY;
    CreateDestroyDisplaySelectNpcDropDownBox();
...
function BtnQuestDebugTextEntryOkBtnButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function ScrollQuestListRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    ...
    // qqq
    String = swprintf("%s %s %d \"%s\" ( %s )", QuestDebugText[Enum167.QUEST_DBS_ENTER_NEW_VALUE], QuestDebugText[Enum167.QUEST_DBS_QUEST_NUM], gubCurQuestSelected, QuestDescText[gubCurQuestSelected], QuestDebugText[Enum167.QUEST_DBS_0_1_2]);
...
function ScrollFactListRegionCallBack(pRegion: MOUSE_REGION, iReason: INT32): void {
  if (iReason & MSYS_CALLBACK_REASON_INIT) {
  } else if (iReason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    let String: string /* wchar_t[512] */;
    ...
    if (FactDescText[gusCurFactSelected][0] == '\0')
      String = swprintf("%s %s %d \"%s\" ( %s )", QuestDebugText[Enum167.QUEST_DBS_ENTER_NEW_VALUE], QuestDebugText[Enum167.QUEST_DBS_FACT_NUM], gusCurFactSelected, QuestDebugText[Enum167.QUEST_DBS_NO_TEXT], QuestDebugText[Enum167.QUEST_DBS_0_1]);
    else
      String = swprintf("%s %s %d \"%s\" ( %s )", QuestDebugText[Enum167.QUEST_DBS_ENTER_NEW_VALUE], QuestDebugText[Enum167.QUEST_DBS_FACT_NUM], gusCurFactSelected, FactDescText[gusCurFactSelected], QuestDebugText[Enum167.QUEST_DBS_0_1]);
...
/* static */ let CreateDestroyDisplayNPCInventoryPopup__fMouseRegionCreated: boolean = false;
function CreateDestroyDisplayNPCInventoryPopup(ubAction: UINT8): void {
  ...
  switch (ubAction) {
    ...
    case Enum166.QD_DROP_DOWN_CREATE:
      ...
      if (CreateDestroyDisplayNPCInventoryPopup__fMouseRegionCreated)
        break;
...
function BtnQuestDebugNPCInventOkBtnButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function BtnQuestDebugAllOrSectorNPCToggleCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    if (gfUseLocalNPCs) {
      gfUseLocalNPCs = false;
...
function BtnQDPgDownButtonButtonCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_DWN) {
    btn.uiFlags |= BUTTON_CLICKED_ON;
    InvalidateRegion(btn.Area.RegionTopLeftX, btn.Area.RegionTopLeftY, btn.Area.RegionBottomRightX, btn.Area.RegionBottomRightY);
  }
...
function DoQDSMessageBox(ubStyle: UINT8, zString: string /* Pointer<INT16> */, uiExitScreen: UINT32, ubFlags: UINT8, ReturnCallback: MSGBOX_CALLBACK | null): boolean {
...
function StartMercTalkingFromQuoteNum(iQuoteToStartTalkingFrom: INT32): void {
  ...
  if (iQuoteToStartTalkingFrom < 0 || iQuoteToStartTalkingFrom > uiMaxNumberOfQuotes) {
    zTemp = swprintf("Please enter a value between 0 and %d", uiMaxNumberOfQuotes);
    DoQDSMessageBox(Enum24.MSG_BOX_BASIC_STYLE, zTemp, Enum26.QUEST_DEBUG_SCREEN, MSG_BOX_FLAG_OK, null);
  } else {
  ...
  // create a mask to block out the screen
  if (!gfBackgroundMaskEnabled) {
    MSYS_DefineRegion(gQuestTextEntryDebugDisableScreenRegion, 0, 0, 640, 480, MSYS_PRIORITY_HIGH + 3, Enum317.CURSOR_LAPTOP_SCREEN, MSYS_NO_CALLBACK, QuestDebugTextEntryDisableScreenRegionCallBack);
    MSYS_AddRegion(gQuestTextEntryDebugDisableScreenRegion);
    gfBackgroundMaskEnabled = true;
  }
...
function BtnQuestDebugAddNpcToTeamToggleCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    if (gfAddNpcToTeam)
...
function BtnQuestDebugRPCSaySectorDescToggleCallback(btn: GUI_BUTTON, reason: INT32): void {
  if (reason & MSYS_CALLBACK_REASON_LBUTTON_UP) {
    if (gfRpcToSaySectorDesc)
